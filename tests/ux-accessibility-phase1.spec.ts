...
test.describe('UX + Accessibility phase 1', () => {
  ...
  test('exposes a keyboard and screen-reader friendly primary workflow', async ({ page }) => {
    ...
    await expect(page.locator('.result-card')).toHaveAttribute('aria-live', 'polite');

    await page.keyboard.press('Tab');
    await expect(page.locator('.language-link')).toBeFocused();
  });

  test('keeps Arabic image compressor in RTL with the same focus contract', async ({ page }) => {
