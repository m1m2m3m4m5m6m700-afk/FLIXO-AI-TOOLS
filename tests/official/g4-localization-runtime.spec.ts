...
const isExpectedNavigationAbort = (request: { url(): string; failure(): { errorText?: string } | null }) => {
  ...
  if (failure?.errorText === 'NS_BINDING_ABORTED') {
    try {
      ...
      if (url.origin === 'http://127.0.0.1:3000' && (url.pathname === '/logo.svg' || url.pathname === '/flixo-logo.svg' || url.pathname === '/flixo-logo.jpg' || url.pathname.startsWith('/assets/'))) return true;
    } catch {
      return false;
    }
  }
...
type ConsoleMessageLike = { type(): string; text(): string; args(): Array<{ jsonValue(): Promise<unknown> }> };
...
for (const pathname of routes) {
  test(`G4 official all-public-route localization/SEO contract — ${pathname}`, async ({ page }) => {
    ...
    page.on('console', (message) => {
      if (message.type() === 'error') consoleErrorPromises.push(serializeConsoleError(message).then((text) => runtimeErrors.push(`console: ${text}`)));
    });
