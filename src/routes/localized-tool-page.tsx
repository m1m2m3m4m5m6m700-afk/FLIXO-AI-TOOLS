...
export function LocalizedToolPage() {
  ...
  const locale: Locale = typeof params.locale === 'string' && isLocale(params.locale) ? params.locale : 'en';
  ...
  if (typeof params.locale !== 'string' || !isLocale(params.locale) || !LOCALES.includes(params.locale) || !toolId) {
    return <main lang={locale} dir={isRtl ? 'rtl' : 'ltr'} className="tool-page-modern"><div className="tool-page-modern__body"><section className="tool-page-modern__hero"><p className="tool-page-modern__eyebrow">FLIXO · TOOLS</p><h1 ref={headingRef} tabIndex={-1} className="tool-page-modern__title">{copy.notFound}</h1></section></div></main>;
  }
  ...
  return <main lang={seo.languageTag} dir={seo.direction} className="tool-page-modern">
    ...
    <div className="tool-page-modern__body">
      ...
      <div className="tool-page-modern__breadcrumbs" aria-label={copy.about}><a className="tool-page-modern__crumb" href={homeUrl}>FLIXO</a><span className="tool-page-modern__crumb-sep">/</span><span className="tool-page-modern__crumb">{seo.tool.category}</span><span className="tool-page-modern__crumb-sep">/</span><span className="tool-page-modern__crumb" aria-current="page">{seo.tool.title}</span></div>
      <header className="tool-page-modern__hero"><div className="tool-page-modern__hero-grid"><div><p className="tool-page-modern__eyebrow">FLIXO · {seo.tool.category.toUpperCase()}</p><h1 ref={headingRef} tabIndex={-1} className="tool-page-modern__title">{seo.title.replace(' | FLIXO', '')}</h1><p className="tool-page-modern__description">{seo.description}</p><div className="tool-page-modern__meta"><div className="tool-page-modern__meta-row"><span className="tool-page-modern__badge"><span className="tool-page-modern__badge-dot" /> {copy.ready}</span><span className="tool-page-modern__chip">{copy.language}: {seo.languageTag}</span><span className="tool-page-modern__chip">{seo.tool.category}</span></div><div className={`tool-page-modern__privacy ${privacy.mode === 'local' ? 'tool-page-modern__privacy--local' : 'tool-page-modern__privacy--remote'}`} role="status" aria-label={privacy.label}><span aria-hidden="true">{privacy.mode === 'local' ? '●' : '↗'}</span><strong>{privacy.label}</strong><span>{privacy.detail}</span></div></div></div></div></header>
      <section className="tool-page-modern__workspace" aria-label={seo.tool.title} aria-busy="false"><div className="tool-page-modern__workspace-bar"><span className="tool-page-modern__status"><span className="tool-page-modern__status-led" /> {copy.workspace}</span><span>{seo.tool.id}</span></div><div className="tool-page-modern__tool-host" aria-live="polite"><Suspense fallback={<div className="tool-page-modern__loading" role="status" aria-live="polite">{copy.loading}</div>}><ToolComponent locale={locale} /></Suspense></div></section>
