...
export function LocalizedQuickFlowPage() {
    ...
    return (
      <main lang={locale} dir={direction}>
        <div className="image-tool-container">
          <h1>{copy.missing}</h1>
          <a className="primary-button" href={homeHref}>
            {copy.back}
          </a>
        </div>
      </main>
  ...
  return (
    <main className="image-tool-shell" lang={locale} dir={direction}>
      <div className="image-tool-container">
        <a href={homeHref} className="language-link">
          ← FLIXO
        </a>
        <p className="image-tool-eyebrow" style={{ marginTop: 28 }}>
          {copy.eyebrow}
        </p>
        ...
        <section className="compressor-card" aria-label={copy.runLabel}>
          ...
          <div style={{ marginTop: 18 }}>
            <strong>{percent}%</strong> · {copy.progress}
            <div
              aria-hidden="true"
              style={{
                height: 8,
        ...
        {result && (
          <section className="result-card" style={{ marginTop: 20 }}>
            ...
            <a
              ...
              href={resultUrl}
              download={`flixo-${workflow.id}.${extensionForMime(result.type)}`}
            >
              {copy.download}
            </a>
          </section>
