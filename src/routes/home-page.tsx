...
function renderHeroTitle(value: string) { const match = /^([\s\S]*?)<span>([\s\S]*?)<\/span>([\s\S]*)$/.exec(value); if (!match) return value; return <>{match[1]}<span>{match[2]}</span>{match[3]}</>; }
...
export function HomePage({ locale = 'en' as Locale }: { locale?: Locale }) {
  ...
  return (
    <main className="home-shell" lang={localeMetadata.languageTag} dir={localeMetadata.direction}>
      ...
      <nav className="home-nav" aria-label={copy.ariaPrimary}><div className="home-container home-nav-inner"><Link className="home-brand" to="/" aria-label={copy.ariaHome}><img src="/flixo-logo.svg" alt="FLIXO AI Tools" width={40} height={40} /></Link><div className="home-nav-links"><a href="#tools">{locale === 'ar' ? 'أدوات الصور' : copy.nav.tools}</a><a href={`/${locale}/pix`}>{FILTER_LABELS[locale] ?? copy.nav.categories}</a></div><label className="sr-only" htmlFor="home-language">{copy.nav.switch}</label><select id="home-language" className="home-nav-language" value={locale} aria-label={copy.nav.switch} onChange={(event) => { const nextLocale = event.target.value as Locale; void navigate(nextLocale === 'en' ? { to: '/' } : { to: '/$locale', params: { locale: nextLocale } }); }}>{LOCALES.map((code) => <option key={code} value={code}>{LANGUAGE_LABELS[code] ?? code}</option>)}</select></div></nav>
