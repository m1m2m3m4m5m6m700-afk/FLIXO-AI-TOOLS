  ...
  return (
    <main className="home-shell" lang={localeMetadata.languageTag} dir={localeMetadata.direction}>
      ...
      <div className="home-container home-content">
        ...
        <section className="home-quick-drop" aria-labelledby="quick-drop-title"><div><span className="image-tool-eyebrow">{copy.quickDrop}</span><h2 id="quick-drop-title">{copy.quickDropTitle}</h2><p>{copy.quickDropLead}</p></div><label className="home-drop-zone"><input type="file" aria-label={copy.dropChoose} onChange={(event) => setDropRecommendation(event.target.files?.[0] ? recommendTool(event.target.files[0], locale) : null)} /><strong>{copy.dropChoose}</strong><span>{copy.dropSupport}</span></label>{dropRecommendation && <div className="drop-result"><div><span>{copy.suggestedTool}</span><strong>{dropRecommendation.title}</strong></div><Link className="primary-button" to={dropRecommendation.path}>{copy.openTool}</Link></div>}</section>
