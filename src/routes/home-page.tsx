...
const LANGUAGE_LABELS: Record<Locale, string> = {
  ar: 'العربية', en: 'English', es: 'Español', fr: 'Français', de: 'Deutsch', ru: 'Русский', zh: '中文', hi: 'हिन्दी', id: 'Bahasa Indonesia', ur: 'اردو', ja: '日本語', pt: 'Português', it: 'Italiano', ko: '한국어', nl: 'Nederlands', pl: 'Polski', tr: 'Türkçe', vi: 'Tiếng Việt', th: 'ไทย', sv: 'Svenska',
};
...
function toLocalizedTool(tool: ToolConfig, locale: Locale): ToolCardProps {
  ...
  const localizedCategory = localizeToolCategory(locale, 'Images');
  const localizedDescription = localizeToolDescription(locale, localizedTitle, 'Images');
...
export function HomePage({ locale = 'en' as Locale }: { locale?: Locale }) {
  ...
  useEffect(() => { let active = true; void loadHomeCopy(locale).then((nextCopy) => { if (active) setCopy(nextCopy); }); return () => { active = false; }; }, [locale]);
  useEffect(() => { const handleKeyDown = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); setPaletteOpen(true); } }; window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, []);
  ...
  const filteredTools = useMemo(() => { const normalized = query.trim().toLowerCase(); return localizedTools.filter((tool) => { const matchesCategory = selectedCategory === 'All' || selectedCategory === tool.category; const haystack = `${tool.id} ${tool.title} ${tool.description} ${tool.categoryLabel}`.toLowerCase(); return matchesCategory && (!normalized || haystack.includes(normalized)); }); }, [localizedTools, query, selectedCategory]);
  ...
  return (
    <main className="home-shell" lang={localeMetadata.languageTag} dir={localeMetadata.direction}>
      ...
      <nav className="home-nav" aria-label={copy.ariaPrimary}><div className="home-container home-nav-inner"><Link className="home-brand" to="/" aria-label={copy.ariaHome}>FLIXO</Link><div className="home-nav-links"><a href="#tools">{copy.nav.tools}</a><a href="#categories">{copy.nav.categories}</a><a href="#privacy">{copy.nav.privacy}</a></div><label className="sr-only" htmlFor="home-language">{copy.nav.switch}</label><select id="home-language" className="home-nav-language" value={locale} aria-label={copy.nav.switch} onChange={(event) => { void navigate({ to: `/${event.target.value}` }); }}>{LOCALES.map((code) => <option key={code} value={code}>{LANGUAGE_LABELS[code]}</option>)}</select></div></nav>
