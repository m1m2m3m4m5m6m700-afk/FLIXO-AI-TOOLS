...
function renderHeroTitle(value: string): ReactNode {
  const spanMatch = /^([\s\S]*?)<span>([\s\S]*?)<\/span>$/.exec(value);
  if (!spanMatch) return value;
  return <>{spanMatch[1]}<span>{spanMatch[2]}</span></>;
}
