...
const LANGUAGE_LABELS: Record<Locale, string> = {
  ...
  ru: 'Русский', sv: 'Svenska', th: 'ไทย', tr: 'Türkçe', vi: 'Tiếng Việt', zh: '中文', ur: 'اردو',
};
...
function toLocalizedTool(tool: ToolConfig, locale: Locale): ToolCardProps {
  ...
  const localizedDescription = localizeToolDescription(locale, localizedTitle, tool.category);
  return {
    id: tool.id,
...
function renderHeroTitle(value: string) {
  const match = /^(.*?<span>)(.*?)(<\/span>.*)$/s.exec(value);
...
export function HomePage({ locale = 'en' as Locale }: { locale?: Locale }) {
  ...
  const categories = useMemo(() => ['All', 'Images'], []);
