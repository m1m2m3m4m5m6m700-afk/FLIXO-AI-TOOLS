...
function AdminPage() {
  ...
  const updateSurvey = (patch: Partial<Survey>) => {
    ...
    persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, ...patch, updatedAt: new Date().toISOString() } : survey), 'Survey saved', 'update_survey', selectedSurvey.title);
  };
  ...
  const addQuestion = () => {
    ...
    persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: [...survey.questions, question], updatedAt: new Date().toISOString() } : survey), 'Question added', 'add_question', question.label);
    setQuestionLabel('');
  };
  const updateQuestion = (questionId: string, patch: Partial<SurveyQuestion>) => {
    if (!selectedSurvey) return;
    persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: survey.questions.map((question) => question.id === questionId ? { ...question, ...patch } : question), updatedAt: new Date().toISOString() } : survey), 'Question updated', 'update_question', selectedSurvey.title);
  };
  const deleteQuestion = (questionId: string) => {
    if (!selectedSurvey) return;
    persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: survey.questions.filter((question) => question.id !== questionId), updatedAt: new Date().toISOString() } : survey), 'Question deleted', 'delete_question', selectedSurvey.title);
  };
  const signOut = async () => {
    setBusy(true);
    try {
