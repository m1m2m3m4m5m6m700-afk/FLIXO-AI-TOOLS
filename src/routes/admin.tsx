...
import { getAdminSessionStatus, logoutAdmin } from '@/lib/admin/auth.server';
import './admin.css';
...
const tabs: Array<{ id: AdminTab; label: string; icon: typeof BarChart3 }> = [
  { id: 'overview', label: 'Overview', icon: BarChart3 }, { id: 'surveys', label: 'Surveys', icon: ClipboardList },
  { id: 'responses', label: 'Responses', icon: FileText }, { id: 'roles', label: 'Roles', icon: Users },
  { id: 'audit', label: 'Audit log', icon: Shield }, { id: 'settings', label: 'Settings', icon: Shield },
];
const questionTypes: Array<{ value: SurveyQuestionType; label: string }> = [
  { value: 'text', label: 'Short text' }, { value: 'textarea', label: 'Long text' }, { value: 'rating', label: 'Rating 1–5' },
  { value: 'select', label: 'Single choice' }, { value: 'multiselect', label: 'Multiple choice' }, { value: 'boolean', label: 'Yes / No' },
];
...
function AdminPage() {
  ...
  const stats = {
    total: surveys.length, active: surveys.filter((survey) => survey.status === 'active').length,
    questions: surveys.reduce((count, survey) => count + survey.questions.length, 0), responses: responses.length,
  };

  const persist = (next: Survey[], message: string, action: string, target: string) => {
    saveSurveys(next); setSurveys(next); audit(action, target); setAuditEvents(getAuditEvents()); setNotice(message);
  };
  const addSurvey = () => { const survey = createSurvey(newTitle, newDescription); persist([survey, ...surveys], 'Survey created', 'create_survey', survey.title); setSelectedId(survey.id); setNewTitle(''); setNewDescription(''); };
  const updateSurvey = (patch: Partial<Survey>) => { if (!selectedSurvey) return; persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, ...patch, updatedAt: new Date().toISOString() } : survey), 'Survey saved', 'update_survey', selectedSurvey.title); };
  const deleteSurvey = (survey: Survey) => { const next = surveys.filter((item) => item.id !== survey.id); persist(next, 'Survey deleted', 'delete_survey', survey.title); setSelectedId(next[0]?.id ?? ''); };
  const addQuestion = () => { if (!selectedSurvey || !questionLabel.trim()) return; const question = createQuestion(questionLabel, questionType); persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: [...survey.questions, question], updatedAt: new Date().toISOString() } : survey), 'Question added', 'add_question', question.label); setQuestionLabel(''); };
  const updateQuestion = (questionId: string, patch: Partial<SurveyQuestion>) => { if (!selectedSurvey) return; persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: survey.questions.map((question) => question.id === questionId ? { ...question, ...patch } : question), updatedAt: new Date().toISOString() } : survey), 'Question updated', 'update_question', selectedSurvey.title); };
  const deleteQuestion = (questionId: string) => { if (!selectedSurvey) return; persist(surveys.map((survey) => survey.id === selectedSurvey.id ? { ...survey, questions: survey.questions.filter((question) => question.id !== questionId), updatedAt: new Date().toISOString() } : survey), 'Question deleted', 'delete_question', selectedSurvey.title); };
  const signOut = async () => { setBusy(true); try { await logoutAdmin(); window.location.assign('/admin/login'); } finally { setBusy(false); } };
