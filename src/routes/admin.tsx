...
import { BarChart3, ClipboardList, Download, FileText, Plus, Shield, Trash2, Users } from 'lucide-react';
import {
  audit,
  createQuestion,
  createSurvey,
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        ...
        <div className="admin-security-card"><Shield size={18} /><strong>Production security</strong><span>This UI deliberately contains no client-side admin secret. Connect server authentication before exposing it publicly.</span></div>
      </aside>

      <section className="admin-main">
        <header className="admin-topbar"><div><span className="admin-eyebrow">CONTROL CENTER</span><h1>FLIXO administration</h1></div><div className="admin-local-badge">Local workspace</div></header>
        ...
        {tab === 'overview' && (
          <section className="admin-content">
            <div className="admin-grid admin-grid-4">
              <StatCard label="Surveys" value={stats.total} icon={<ClipboardList size={18} />} />
              <StatCard label="Active" value={stats.active} icon={<BarChart3 size={18} />} />
              <StatCard label="Questions" value={stats.questions} icon={<FileText size={18} />} />
              <StatCard label="Responses" value={stats.responses} icon={<Users size={18} />} />
            </div>
            <div className="admin-grid admin-grid-2">
              <Panel title="Survey coverage" subtitle="Every questionnaire is modeled explicitly; no fake response counts are injected.">
                <div className="admin-bars">{surveys.map((survey) => <div key={survey.id} className="admin-bar-row"><span>{survey.title}</span><div><i style={{ width: `${Math.max(6, Math.min(100, survey.questions.length * 16))}%` }} /></div><strong>{survey.questions.length}</strong></div>)}</div>
              </Panel>
              <Panel title="Recent admin activity" subtitle="Local audit log for create/update/delete operations.">
                <div className="admin-list">{auditEvents.slice(0, 6).map((event) => <div key={event.id}><strong>{event.action}</strong><span>{event.target}</span><time>{new Date(event.at).toLocaleString()}</time></div>)}{auditEvents.length === 0 && <EmptyState text="No administrative actions yet." />}</div>
              </Panel>
            </div>
          </section>
        )}
        ...
        {tab === 'surveys' && (
          <section className="admin-content">
            <div className="admin-grid admin-grid-surveys">
              <Panel title="All surveys" subtitle="Create, activate, archive and select a questionnaire.">
                <div className="admin-toolbar"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search surveys…" aria-label="Search surveys" /><button type="button" className="admin-primary" onClick={addSurvey}><Plus size={16} />Create</button></div>
                <div className="admin-list">{filteredSurveys.map((survey) => <button key={survey.id} type="button" className={selectedSurvey?.id === survey.id ? 'admin-survey-row selected' : 'admin-survey-row'} onClick={() => setSelectedId(survey.id)}><span><strong>{survey.title}</strong><small>{survey.description}</small></span><em className={`status-${survey.status}`}>{survey.status}</em></button>)}{filteredSurveys.length === 0 && <EmptyState text="No surveys match your search." />}</div>
                <div className="admin-create-box"><input value={newTitle} onChange={(event) => setNewTitle(event.target.value)} placeholder="New survey title" /><textarea value={newDescription} onChange={(event) => setNewDescription(event.target.value)} placeholder="Purpose and audience" rows={3} /><button type="button" className="admin-secondary" onClick={addSurvey}>Add survey</button></div>
              </Panel>

              <Panel title={selectedSurvey?.title ?? 'Survey builder'} subtitle={selectedSurvey?.description ?? 'Select or create a survey to begin.'}>
                {selectedSurvey ? <>
                  <div className="admin-form-grid"><label>Status<select value={selectedSurvey.status} onChange={(event) => updateSurvey({ status: event.target.value as Survey['status'] })}><option value="draft">Draft</option><option value="active">Active</option><option value="archived">Archived</option></select></label><label>Audience<select value={selectedSurvey.audience} onChange={(event) => updateSurvey({ audience: event.target.value as Survey['audience'] })}><option value="all">Everyone</option><option value="signed-in">Signed-in</option><option value="tool-users">Tool users</option></select></label></div>
                  <div className="admin-section-heading"><div><strong>Questions</strong><span>{selectedSurvey.questions.length} configured</span></div><button type="button" className="admin-danger-ghost" onClick={() => deleteSurvey(selectedSurvey)}><Trash2 size={15} />Delete survey</button></div>
                  <div className="admin-question-list">{selectedSurvey.questions.map((question, index) => <div key={question.id} className="admin-question"><div className="admin-question-index">{index + 1}</div><div className="admin-question-fields"><input value={question.label} onChange={(event) => updateQuestion(question.id, { label: event.target.value })} aria-label={`Question ${index + 1}`} /><div className="admin-form-grid"><label>Type<select value={question.type} onChange={(event) => updateQuestion(question.id, { type: event.target.value as SurveyQuestionType })}>{questionTypes.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select></label><label className="admin-checkbox"><input type="checkbox" checked={question.required} onChange={(event) => updateQuestion(question.id, { required: event.target.checked })} />Required</label></div></div><button type="button" className="admin-icon-button" onClick={() => deleteQuestion(question.id)} aria-label={`Delete question ${index + 1}`}><Trash2 size={15} /></button></div>)}</div>
                  <div className="admin-add-question"><input value={questionLabel} onChange={(event) => setQuestionLabel(event.target.value)} placeholder="Question text" /><select value={questionType} onChange={(event) => setQuestionType(event.target.value as SurveyQuestionType)}>{questionTypes.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select><button type="button" className="admin-primary" onClick={addQuestion}><Plus size={16} />Add question</button></div>
                </> : <EmptyState text="Create your first survey to start building questions." />}
              </Panel>
            </div>
          </section>
        )}
        ...
        {tab === 'settings' && <section className="admin-content"><Panel title="Administration settings" subtitle="Safe defaults for privacy and production hardening."><div className="admin-setting-list"><div><strong>Admin indexability</strong><span>Route sends noindex/nofollow metadata.</span><b>Protected</b></div><div><strong>Response retention</strong><span>Keep only the minimum data required for product decisions.</span><b>Policy required</b></div><div><strong>Authentication</strong><span>Do not use client-side secrets. Connect a server-side identity provider before production exposure.</span><b>Backend required</b></div><div><strong>Export</strong><span>CSV export runs only over locally available response records.</span><b>Enabled</b></div></div></Panel></section>}
      </section>
    </main>
  );
}
