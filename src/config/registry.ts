...
function assertToolRegistryContract(tools: readonly ToolDefinition[]): void {
  ...
  for (const tool of tools) {
    ...
    if (!tool.capability.parameterSchema) throw new Error(`Capability parameter schema is missing: ${tool.id}`);
    if (!tool.capability.verifier) throw new Error(`Capability verifier is missing: ${tool.id}`);
