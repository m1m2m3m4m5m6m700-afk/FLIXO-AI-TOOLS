...
import { AI_CAPTIONER_UI } from './ai-captioner-ui-i18n';

function translateValue(locale: Locale, value: string): string {
  if (locale === 'en') return value;
  const trimmed = value.trim();
  const exact = AI_CAPTIONER_UI[trimmed]?.[locale];
...
export function installAiCaptionerUiRuntimeLocalization(): () => void {
