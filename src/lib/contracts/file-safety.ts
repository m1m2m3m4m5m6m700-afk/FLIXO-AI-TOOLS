export type MagicByteSegment = { bytes: readonly number[]; offset: number };
export type MagicByteSignature = { name: string; bytes: readonly number[]; offset?: number; segments?: readonly MagicByteSegment[] };
...
export type FileSafetyResult = { safe: boolean; failures: string[] };
export type ArchiveEntry = { name: string; compressedBytes?: number; uncompressedBytes?: number; isSymlink?: boolean; nestedEntries?: readonly ArchiveEntry[] };
export type ArchiveSafetyPolicy = { maxEntries: number; maxUncompressedBytes: number; maxDepth: number; maxCompressionRatio?: number };
...
export const MAGIC_BYTE_SIGNATURES: Readonly<Record<string, MagicByteSignature>> = Object.freeze({
  ...
  avif: { name: 'AVIF/ISO-BMFF', bytes: ftypSignature, offset: 4 },
});
...
function isUnsafeName(name: string): boolean {
  ...
  if (/^[A-Za-z]:($|[\\/])/.test(name) || /^[/\\]/.test(name)) return true;
...
export function detectZipBombRisk(compressedSize: number, uncompressedSize: number, maxCompressionRatio = 40): { isBomb: boolean; reason?: string } {
  if (!Number.isFinite(compressedSize) || !Number.isFinite(uncompressedSize) || !Number.isFinite(maxCompressionRatio)) return { isBomb: true, reason: 'Invalid ZIP bomb detection size or ratio input.' };
  ...
  return ratio > maxCompressionRatio
    ? { isBomb: true, reason: `Potential ZIP bomb detected: Expansion ratio of ${ratio.toFixed(1)}x exceeds safety threshold (${maxCompressionRatio}x).` }
    : { isBomb: false };
}
...
export function verifyMagicBytesMatch(headerBytes: Uint8Array, allowedSignatures?: readonly string[]): boolean {
  ...
  return allowedSignatures.some((sigName) => {
    const signature = MAGIC_BYTE_SIGNATURES[sigName.toLowerCase()];
    return signature?.bytes.every((byte, index) => headerBytes[index] === byte) ?? false;
  });
}
...
function validateContent(content: Uint8Array, validation: ContentValidation, failures: string[]): void {
  let text: string;
  try { text = new TextDecoder('utf-8', { fatal: true }).decode(content).replace(/^\uFEFF/, ''); }
  catch { failures.push('input content is not valid UTF-8'); return; }
  if (validation === 'json') { try { JSON.parse(text); } catch { failures.push('input JSON content is malformed'); } }
}
...
export function validateFileSafety(input: FileSafetyInput, policy: FileSafetyPolicy): FileSafetyResult {
  ...
  if (!name || isUnsafeName(name)) failures.push('file name must be a single safe relative name');
  ...
  if (input.width !== undefined || input.height !== undefined) {
    if (!Number.isInteger(input.width) || Number(input.width) < 1) failures.push('width must be a positive integer');
    if (!Number.isInteger(input.height) || Number(input.height) < 1) failures.push('height must be a positive integer');
...
export function validateArchiveEntries(entries: readonly ArchiveEntry[], policy: ArchiveSafetyPolicy): FileSafetyResult {
  ...
  const visit = (current: readonly ArchiveEntry[], parentDepth: number): void => {
    for (const entry of current) {
      if (stopped) return;
      totalEntries += 1;
      if (totalEntries > policy.maxEntries) { failures.push('archive exceeds the maximum entry count'); stopped = true; return; }
      ...
      if (entry.compressedBytes !== undefined && (!Number.isInteger(entry.compressedBytes) || entry.compressedBytes < 1)) failures.push(`archive entry has an invalid compressed size: ${entry.name}`);
      if (entry.uncompressedBytes !== undefined) {
        if (!Number.isInteger(entry.uncompressedBytes) || entry.uncompressedBytes < 0) failures.push(`archive entry has an invalid uncompressed size: ${entry.name}`);
          ...
          if (totalBytes > policy.maxUncompressedBytes) { failures.push('archive exceeds the maximum uncompressed size'); stopped = true; return; }
