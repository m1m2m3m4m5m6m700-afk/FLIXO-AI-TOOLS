...
async function imageBitmap(blob: Blob, owner: DisposableResourceOwner, signal: AbortSignal) {
  ...
  const url = owner.objectUrl('pipeline-image-source', blob); const image = new Image();
  try { image.src = url; await image.decode(); throwIfAborted(signal); return image; } finally { await owner.dispose('pipeline-image-source'); }
}
function canvasToBlob(canvas: HTMLCanvasElement, type = 'image/png', quality = 0.94) { return new Promise<Blob>((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Image encoding failed.')), type, quality)); }
async function effects(blob: Blob, params?: PipelineParams, owner?: DisposableResourceOwner, signal?: AbortSignal) {
  const resourceOwner = owner ?? new DisposableResourceOwner();
  const resourceSignal = signal ?? new AbortController().signal;
  const image = await imageBitmap(blob, resourceOwner, resourceSignal); const canvas = document.createElement('canvas'); canvas.width = image.width; canvas.height = image.height;
  ...
  try { throwIfAborted(resourceSignal); if (!values.every(Number.isFinite)) throw new Error('Image effect parameters must be finite numbers.'); ctx.filter = `brightness(${values[0]}%) contrast(${values[1]}%) saturate(${values[2]}%) grayscale(${values[3]}%)`; ctx.drawImage(image, 0, 0); throwIfAborted(resourceSignal); return canvasToBlob(canvas); }
  finally { if ('close' in image && typeof image.close === 'function') image.close(); if (!owner) await resourceOwner.disposeAll(); }
}
async function processToolStep(toolId: ExecutablePipelineToolId, inputBlob: Blob, params: PipelineParams, owner: DisposableResourceOwner, signal: AbortSignal) {
  ...
  switch (toolId) {
    ...
    case 'image-compressor': return (await compressImage(asFile(inputBlob), { quality: Number(params.quality ?? 0.82), format: String(params.format ?? 'image/webp') as 'image/webp' | 'image/jpeg' | 'image/png', targetSizeKB: Number(params.targetSizeKB ?? 0) || undefined }, owner, signal)).blob;
...
export async function runWorkflowPipeline(initialFile: File, plan: ExecutionPlan, onProgress: (progress: PipelineProgress) => void, signal?: AbortSignal): Promise<Blob> {
  ...
  const resourceSignal = signal ?? controller!.signal;
  let currentBlob: Blob = initialFile;
  try {
    for (let i = 0; i < plan.steps.length; i += 1) {
      throwIfAborted(resourceSignal);
      ...
      currentBlob = await processToolStep(step.toolId as ExecutablePipelineToolId, currentBlob, step.params, owner, resourceSignal);
      throwIfAborted(resourceSignal);
