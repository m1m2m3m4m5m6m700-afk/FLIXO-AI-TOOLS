...
function reportDisposalFailure(error: unknown): void { console.error('FLIXO resource disposal failed.', error); }
export class DisposableResourceOwner {
  ...
  track<T>(key: string, resource: T, dispose: (resource: T) => void | Promise<void>): T {
    ...
    if (previous) {
      ...
      void pending.then(() => { this.pendingDisposals.delete(pending); }, (error) => { this.pendingDisposals.delete(pending); reportDisposalFailure(error); });
    }
  ...
  async disposeAll(): Promise<void> {
    ...
    try {
      for (const key of entries) {
        try { await this.dispose(key); } catch (error) { errors.push(error); reportDisposalFailure(error); }
      }
      const pendingResults = await Promise.allSettled(Array.from(this.pendingDisposals));
      for (const result of pendingResults) if (result.status === 'rejected') errors.push(result.reason);
  ...
  objectUrl(key: string, value: Blob | MediaSource): string { const url = URL.createObjectURL(value); this.track(key, url, (ownedUrl) => URL.revokeObjectURL(ownedUrl)); return url; }
  worker(key: string, worker: Worker): Worker { return this.track(key, worker, (ownedWorker) => ownedWorker.terminate()); }
  audioContext<T extends AudioContext>(key: string, context: T): T { return this.track(key, context, async (ownedContext) => { if (ownedContext.state !== 'closed') await ownedContext.close(); }); }
}
export function useDisposableResourceOwner(): DisposableResourceOwner { const [owner] = useState(() => new DisposableResourceOwner()); useEffect(() => () => { void owner.disposeAll().catch(reportDisposalFailure); }, [owner]); return owner; }
