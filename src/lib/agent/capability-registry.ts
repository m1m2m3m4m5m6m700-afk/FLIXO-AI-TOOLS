...
function executionModeFor(toolId: string): ExecutionMode { return toolId === 'ai-image-generator' || toolId === 'photo-colorizer' ? 'CLOUD' : 'LOCAL'; }
function stateFor(toolId: string, isReady: boolean): CapabilityState { if (!isReady) return 'UNAVAILABLE'; if (EXECUTABLE_IDS.has(toolId)) return 'EXECUTABLE'; if (toolId in TOOL_INTENTS) return 'PLANNABLE'; return 'RECOGNIZED'; }
...
const formatVerifier = async (_inputBlob: Blob, outputBlob: Blob, parameters: CapabilityParameters): Promise<boolean> => {
...
const entries = IMAGE_TOOLS.map((tool) => ({
  ...
  safetyLimits: { maxPixels: DEFAULT_MAX_PIXELS, maxFileSizeBytes: DEFAULT_MAX_FILE_SIZE_BYTES, timeoutMs: DEFAULT_TIMEOUT_MS },
...
export function getCapabilitiesByState(state: CapabilityState): readonly CapabilityContract[] { return CAPABILITY_REGISTRY.filter((capability) => capability.state === state); }
export function getExecutableCapabilityIds(): readonly string[] { return CAPABILITY_REGISTRY.filter((capability) => capability.state === 'EXECUTABLE').map((capability) => capability.id); }
