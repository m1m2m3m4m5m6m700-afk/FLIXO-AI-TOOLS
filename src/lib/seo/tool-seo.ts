...
export function getToolSeo(localeInput: string, toolId: string) {
  ...
  const category = assertToolCategory(tool.category); const label = LOCALE_LABELS[locale];
  ...
  const title = `${localizedTitle} | FLIXO`; const description = localizedDescription; const fallback = FALLBACK_COPY[locale];
