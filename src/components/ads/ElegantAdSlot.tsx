...
const FALLBACK_COPY: Record<string, { title: string; body: string; action: string }> = {
  ...
  zh: { title: '广告覆盖范围正在扩展', body: '此区域用于展示广告。在该语言尚未启用变现时，我们会显示轻量的替代内容。', action: '探索 FLIXO AI' },
  ur: { title: 'FLIXO AI اشتہاری کوریج بڑھا رہا ہے', body: 'یہ جگہ اشتہارات کے لیے مخصوص ہے۔ اس زبان کے لیے آمدنی فعال ہونے تک ہلکا متبادل دکھایا جاتا ہے۔', action: 'FLIXO AI دیکھیں' },
};
...
export function ElegantAdSlot({ slotId = 'default', className = '', monetizedLocales = ['en'], adHref = '/' }: ElegantAdSlotProps) {
  ...
  const copy = FALLBACK_COPY[locale] ?? FALLBACK_COPY.en;
  ...
  return (
    <div ref={slotRef} data-ad-slot={slotId} data-monetized={isMonetized ? 'true' : 'false'} className={`mx-auto w-full max-w-[1480px] px-2 pb-2 sm:px-3 lg:px-4 ${className}`} aria-label="Advertisement">
      <div className="rounded-2xl border border-zinc-200/80 bg-zinc-50/90 p-4 shadow-[0_14px_40px_rgba(0,0,0,0.06)] transition dark:border-white/[0.08] dark:bg-zinc-950/80 dark:shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
        {isMonetized ? (
          <div className="min-h-[90px] text-center" data-ad-provider="native"><div className="flex min-h-[90px] items-center justify-center text-xs text-zinc-400 dark:text-zinc-500"><span>Advertisement</span></div></div>
        ) : (
          <a href={adHref} onClick={() => trackUserMovement('ad_clicked', { slotId, locale, monetized: false })} className="group flex min-h-[90px] items-center justify-between gap-4 rounded-xl px-3 py-2 text-left outline-none transition focus-visible:ring-2 focus-visible:ring-violet-500/70" aria-label={copy.action} dir={locale === 'ar' || locale === 'ur' ? 'rtl' : 'ltr'}>
