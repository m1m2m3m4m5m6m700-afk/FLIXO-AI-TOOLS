    ...
    <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] bg-zinc-950/75 px-3 py-3 backdrop-blur-2xl sm:px-4">
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:justify-end">
        <div className="inline-flex items-center gap-2 rounded-lg border border-white/[0.07] bg-white/[0.035] px-2.5 py-2 font-mono text-[9px] uppercase tracking-[0.12em] text-zinc-400" aria-label={ready ? copy.ready : copy.waiting}>
          ...
          <Activity className="size-3" aria-hidden="true" />{ready ? copy.ready : copy.waiting}
        </div>
        ...
        {onUpload ? <button type="button" onClick={onUpload} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-white/[0.07] bg-white/[0.035] px-2.5 text-xs text-zinc-300 transition hover:-translate-y-px hover:border-violet-300/20 hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/70" aria-label={copy.upload}><Upload className="size-3.5" aria-hidden="true" /><span className="hidden sm:inline">{copy.upload}</span></button> : null}
        {onReset ? <button type="button" onClick={onReset} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-white/[0.07] bg-white/[0.035] px-2.5 text-xs text-zinc-300 transition hover:-translate-y-px hover:border-violet-300/20 hover:bg-white/[0.06] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/70" aria-label={copy.reset}><RotateCcw className="size-3.5" aria-hidden="true" /><span className="hidden sm:inline">{copy.reset}</span></button> : null}
