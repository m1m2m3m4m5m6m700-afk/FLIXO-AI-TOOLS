...
function isTextExcluded(node: Text): boolean {
...
function translateValue(locale: Locale, value: string): string {
  ...
  const exact = UI_PHRASES[trimmed]?.[locale];
  if (exact) return value.replace(trimmed, exact);

  const prefixRules: Array<[string, LocaleMap]> = [
    ['Download ', { ar: 'تنزيل ', es: 'Descargar ', fr: 'Télécharger ', de: 'Herunterladen ', ru: 'Скачать ', zh: '下载 ', hi: 'डाउनलोड ', id: 'Unduh ', ur: 'ڈاؤن لوڈ ', ja: 'ダウンロード ', pt: 'Baixar ', it: 'Scarica ', ko: '다운로드 ', nl: 'Downloaden ', pl: 'Pobierz ', tr: 'İndir ', vi: 'Tải xuống ', th: 'ดาวน์โหลด ', sv: 'Ladda ner ' }],
  ...
  for (const [prefix, translations] of prefixRules) {
    if (value.startsWith(prefix)) return `${translations[locale] ?? prefix}${value.slice(prefix.length)}`;
  }
  return value;
}

function localizeSurface(root: HTMLElement, locale: Locale) {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
  const nodes: Text[] = [];
  while (walker.nextNode()) {
    const node = walker.currentNode as Text;
    if (!node.nodeValue?.trim() || isTextExcluded(node)) continue;
    nodes.push(node);
  }
...
export function AutoLocalizedToolSurface({ locale, children }: Props) {
  useEffect(() => {
    ...
    localizeSurface(root, locale);
    const observer = new MutationObserver(() => localizeSurface(root, locale));
