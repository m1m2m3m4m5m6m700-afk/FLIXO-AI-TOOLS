...
export function TechnicalToolShell({ title, eyebrow = 'FLIXO TOOL ENGINE', description, ready = true, progress, onUpload, onReset, onExport, exportDisabled = false, onCommandMenu, children }: TechnicalToolShellProps) {
  return (
    <div className="relative mx-auto w-full max-w-[1480px] p-2 sm:p-3 lg:p-4">
      <div className="flixo-premium-shell relative overflow-hidden rounded-[22px] border border-white/[0.08] bg-[#050507] shadow-[0_28px_100px_rgba(0,0,0,0.38)] backdrop-blur-2xl">
        ...
        <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] bg-zinc-950/75 px-3 py-3 backdrop-blur-2xl sm:px-4 sm:py-3.5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:justify-end">
            ...
            {onUpload ? <button type="button" onClick={onUpload} className={actionClass} aria-label="Upload"> <Upload className="size-3.5" /> <span className="hidden sm:inline">Upload</span> </button> : null}
            {onReset ? <button type="button" onClick={onReset} className={actionClass} aria-label="Reset"> <RotateCcw className="size-3.5" /> <span className="hidden sm:inline">Reset</span> </button> : null}
      ...
      <div className="mt-2 flex items-center justify-end gap-1.5 px-1 text-[9px] text-zinc-600"><CheckCircle2 className="size-3" />Local-first creative workspace</div>
    </div>
  );
}
