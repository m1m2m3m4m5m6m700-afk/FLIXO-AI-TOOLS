...
export function ArQuickFlowPanel({ copy, workflow, plan, file, resultUrl, resultType, error, busy, percent, currentName, onFileChange, onRun }: Props) {
  return (
    <div dir="rtl" lang="ar" className="image-tool-shell">
      <div className="image-tool-container">
        ...
        <section className="compressor-card" aria-label={copy.runLabel}>
          ...
          <div style={{ marginTop: 18 }}><strong>{percent}%</strong>{currentName ? ` · ${currentName}` : ''}<div aria-hidden="true" style={{ height: 8, marginTop: 8, borderRadius: 999, background: 'var(--surface-muted, #eee)', overflow: 'hidden' }}><div style={{ width: `${percent}%`, height: '100%', transition: 'width 180ms ease' }} /></div></div>
