...
function encode(canvas: HTMLCanvasElement, format: CompressionFormat, quality: number) {
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((result) => (result ? resolve(result) : reject(new Error('Image encoding failed'))), format, Math.min(1, Math.max(0.05, quality)));
  });
}

async function encodeToTarget(canvas: HTMLCanvasElement, format: CompressionFormat, quality: number, targetBytes?: number) {
  if (!targetBytes || format === 'image/png') return { blob: await encode(canvas, format, quality), qualityUsed: quality };
  let low = 0.05; let high = Math.min(1, Math.max(0.05, quality)); let bestBlob: Blob | null = null; let bestQuality = low;
  for (let attempt = 0; attempt < 7; attempt += 1) {
    const candidateQuality = (low + high) / 2; const candidate = await encode(canvas, format, candidateQuality);
    if (candidate.size <= targetBytes) { bestBlob = candidate; bestQuality = candidateQuality; low = candidateQuality; if (candidate.size >= targetBytes * 0.98) break; }
...
type SourceImage = { source: CanvasImageSource; width: number; height: number; cleanup: () => void };
...
async function loadSourceImage(file: File): Promise<SourceImage> {
  ...
  try {
    ...
    return { source: image, width: image.naturalWidth, height: image.naturalHeight, cleanup: () => owner.dispose() };
  } catch (error) {
    owner.dispose();
...
async function compressImageOnMainThread(file: File, options: CompressionOptions): Promise<CompressionResult> {
  ...
  try {
    ...
    const context = canvas.getContext('2d', { alpha: true }); if (!context) throw new Error('Canvas is unavailable');
    context.imageSmoothingEnabled = true; context.imageSmoothingQuality = 'high';
    if (options.format === 'image/jpeg') { context.fillStyle = '#ffffff'; context.fillRect(0, 0, size.width, size.height); }
...
type WorkerResponse = { ok: true; result: CompressionResult } | { ok: false; error: string };
function canUseCompressionWorker(file: File) { return typeof Worker !== 'undefined' && typeof OffscreenCanvas !== 'undefined' && typeof createImageBitmap === 'function' && file.type !== 'image/svg+xml'; }

function compressImageInWorker(file: File, options: CompressionOptions): Promise<CompressionResult> {
  return new Promise((resolve, reject) => {
    const owner = new DisposableResourceOwner();
    const worker = owner.trackWorker(new Worker(new URL('./compressor.worker.ts', import.meta.url), { type: 'module' }));
    const finish = () => owner.dispose();
    worker.onmessage = (event: MessageEvent<WorkerResponse>) => { finish(); if (event.data.ok) resolve(event.data.result); else reject(new Error(event.data.error)); };
    worker.onerror = () => { finish(); reject(new Error('The compression worker failed.')); };
...
export async function compressImage(file: File, options: CompressionOptions): Promise<CompressionResult> {
  ...
  if (canUseCompressionWorker(file)) {
    try { return await compressImageInWorker(file, options); } catch { /* Safe main-thread fallback for browsers with partial worker/canvas support. */ }
  }
