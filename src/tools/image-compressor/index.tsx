  ...
  return (
    <div lang={locale} dir={isArabic ? 'rtl' : 'ltr'} className="image-tool-shell">
      <div className="image-tool-container">
        <header className="image-tool-header"><div><p className="image-tool-eyebrow">FLIXO · IMAGE TOOLS</p><h2 ref={headingRef} tabIndex={-1}>{title}</h2><p className="image-tool-lead">{description}</p></div><a className="language-link" href={isArabic ? '/en/image-compressor' : '/ar/image-compressor'}>{label(locale, 'العربية', 'English')}</a></header>
        <section className="compressor-grid" aria-label={label(locale, 'Image compression tool', 'أداة ضغط الصور')} aria-busy={busy}>
          <div className="compressor-card">
            ...
            <div className="control-grid">
              ...
              <label><span>{label(locale, 'Max width', 'أقصى عرض')}</span><input inputMode="numeric" placeholder={label(locale, 'Auto', 'تلقائي')} value={maxWidth} onChange={(event) => setMaxWidth(event.target.value.replace(/\D/g, ''))} /></label>
              <label><span>{label(locale, 'Max height', 'أقصى ارتفاع')}</span><input inputMode="numeric" placeholder={label(locale, 'Auto', 'تلقائي')} value={maxHeight} onChange={(event) => setMaxHeight(event.target.value.replace(/\D/g, ''))} /></label>
            </div>
