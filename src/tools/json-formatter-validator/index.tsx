      ...
      <div className="space-y-1" style={{ marginLeft: depth * 12 }}>
        {entries.map(([key, child]) => (
          <div key={`${depth}-${key}`}>
            ...
            {child && typeof child === 'object' ? <TreeNode value={child} depth={depth + 1} /> : <span>{JSON.stringify(child)}</span>}
          </div>
        ))}
      </div>
    );
  }
  return <span>{label ? `${label}: ` : ''}{JSON.stringify(value)}</span>;
};
...
export function JsonFormatterValidatorTool() {
  ...
  const downloadOutput = () => {
    ...
    anchor.href = url;
    anchor.download = `flixo-output.${ext}`;
    anchor.click();
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      <header>
        ...
        <p className="mt-2 opacity-80">Format, validate, inspect, convert, copy, and download JSON locally.</p>
      </header>

      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => run('pretty')}>Prettify</button>
        <button type="button" onClick={() => run('minify')}>Minify</button>
        <button type="button" onClick={() => run('yaml')}>YAML</button>
        <button type="button" onClick={() => run('csv')}>CSV</button>
        ...
        <button type="button" onClick={() => setMode((current) => current === 'editor' ? 'tree' : 'editor')}>{mode === 'editor' ? 'Tree View' : 'Editor View'}</button>
