...
export function VideoGifMemeTool() {
  ...
  return (
    <section className="mx-auto flex max-w-3xl flex-col gap-5 rounded-2xl border border-border bg-background p-6 text-foreground">
      ...
      {file ? (<><video className="max-h-80 w-full rounded-xl bg-black" src={URL.createObjectURL(file)} controls muted /><div className="grid gap-4 md:grid-cols-2"><label>Start<input className="mt-1 w-full rounded border p-2" type="number" min={0} max={duration} step={0.1} value={start} onChange={(e) => setStart(Number(e.target.value))} /></label><label>End<input className="mt-1 w-full rounded border p-2" type="number" min={0} max={duration} step={0.1} value={end} onChange={(e) => setEnd(Number(e.target.value))} /></label><label>FPS<input className="mt-1 w-full rounded border p-2" type="number" min={2} max={15} value={fps} onChange={(e) => setFps(Number(e.target.value))} /></label><label>Width<input className="mt-1 w-full rounded border p-2" type="number" min={160} max={720} value={width} onChange={(e) => setWidth(Number(e.target.value))} /></label><label>Top text<input className="mt-1 w-full rounded border p-2" value={topText} onChange={(e) => setTopText(e.target.value)} /></label><label>Bottom text<input className="mt-1 w-full rounded border p-2" value={bottomText} onChange={(e) => setBottomText(e.target.value)} /></label></div><button type="button" disabled={busy} onClick={() => void generate()} className="rounded-xl bg-primary px-4 py-3 font-semibold text-primary-foreground disabled:opacity-50">{busy ? 'Rendering…' : 'Create GIF'}</button></>) : null}
