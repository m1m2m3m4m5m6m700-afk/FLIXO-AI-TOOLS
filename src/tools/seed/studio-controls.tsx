...
export function ToolSection({ value, title, subtitle, icon: Icon, activeCount, children }: ToolSectionProps) {
  ...
  useEffect(() => { if (value !== 'retouch' && value !== 'fx') return; if (triggerRef.current?.getAttribute('data-state') === 'closed') triggerRef.current.click(); }, [value]);
  return (
    <>
      ...
      <Accordion.Item value={value} className="overflow-hidden rounded-2xl border border-white/[0.07] bg-zinc-900/45 shadow-[0_14px_34px_rgba(0,0,0,0.12)]"><Accordion.Header><Accordion.Trigger ref={triggerRef} className="group flex w-full items-center gap-3 px-3.5 py-3.5 text-left outline-none transition hover:bg-white/[0.025] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-400/60"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-indigo-300/10 bg-gradient-to-br from-indigo-500/12 via-violet-500/8 to-cyan-400/5 text-indigo-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]"><Icon className="size-4" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="text-[12px] font-semibold tracking-tight text-zinc-100">{title}</span><span className={`rounded-full border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.13em] ${activeCount > 0 ? 'border-cyan-300/15 bg-cyan-300/[0.06] text-cyan-200' : 'border-white/[0.06] bg-zinc-950/80 text-zinc-600'}`}>{activeCount} active</span></span><span className="mt-0.5 block truncate text-[10px] text-zinc-500">{subtitle}</span></span><ChevronDown className="size-4 shrink-0 text-zinc-600 transition-transform duration-200 group-data-[state=open]:rotate-180" /></Accordion.Trigger></Accordion.Header><Accordion.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down"><div className="space-y-2 border-t border-white/[0.05] bg-zinc-950/20 p-2.5">{children}</div></Accordion.Content></Accordion.Item>
    </>
  );
}
...
export function CurveMiniPreview({ y, ariaLabel = 'Curves preview' }: CurveMiniPreviewProps) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-zinc-950/90 p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"><svg viewBox="0 0 160 96" className="h-24 w-full" role="img" aria-label={ariaLabel}><defs><pattern id="seed-grid" width="16" height="16" patternUnits="userSpaceOnUse"><path d="M 16 0 L 0 0 0 16" fill="none" stroke="rgba(255,255,255,0.045)" strokeWidth="1" /></pattern></defs><rect width="160" height="96" fill="url(#seed-grid)" /><line x1="0" y1="96" x2="160" y2="0" stroke="rgba(255,255,255,0.2)" strokeWidth="1" strokeDasharray="3 3" /><path d={`M0 96 C48 96 50 ${controlY} 80 ${controlY} C110 ${controlY} 112 4 160 0`} fill="none" stroke="rgb(129,140,248)" strokeWidth="2.5" strokeLinecap="round" /><circle cx="80" cy={controlY} r="4" fill="rgb(103,232,249)" /></svg><div className="flex items-center justify-between px-1 pt-1 font-mono text-[8px] uppercase tracking-[0.12em] text-zinc-700"><span>Input</span><span>Midtone</span><span>Output</span></div></div>;
}
