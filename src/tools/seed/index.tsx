  ...
  return (
    <div className="mx-auto flex min-h-[760px] w-full max-w-[1500px] flex-col gap-3 p-2 sm:p-3 lg:p-4">
      ...
      <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_380px]"><section ref={stageRef} className={`relative flex min-h-[560px] min-w-0 items-center justify-center overflow-hidden rounded-2xl border bg-zinc-950 p-3 shadow-[0_24px_70px_rgba(0,0,0,0.24)] transition ${isDragging ? 'border-indigo-400/70 bg-indigo-950/10' : 'border-white/[0.07]'}`} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={handleDragLeave} onDrop={handleDrop}>
          ...
          <FloatingCanvasOverlay zoomLevel={zoomLevel} labels={SEED_OVERLAY_LABELS} canUndo={historyIndex > 0} canRedo={historyIndex < history.length - 1} isFullscreen={isFullscreen} isMobileSheetOpen={false} onZoomIn={handleZoomIn} onZoomOut={handleZoomOut} onResetZoom={handleResetZoom} onUndo={undo} onRedo={redo} onCompareStart={handleCompareStart} onCompareEnd={handleCompareEnd} onToggleFullscreen={toggleFullscreen} />
        </section>
        <aside className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-zinc-950/90 shadow-[0_24px_70px_rgba(0,0,0,0.28)] backdrop-blur-xl"><div className="shrink-0 border-b border-white/[0.06] bg-zinc-950/95 px-3.5 py-3.5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2.5"><span className="flex size-8 items-center justify-center rounded-lg border border-white/[0.07] bg-zinc-900 text-zinc-300"><SlidersHorizontal className="size-4" /></span><div><div className="text-[12px] font-semibold text-zinc-100">Control Matrix</div><div className="font-mono text-[9px] uppercase tracking-[0.12em] text-zinc-600">Non-destructive parameter graph</div></div></div><button type="button" onClick={() => imageInputRef.current?.click()} className="inline-flex size-8 items-center justify-center rounded-lg border border-white/[0.06] bg-zinc-900 text-zinc-500 transition hover:border-white/[0.11] hover:text-white" aria-label="Replace image"><Upload className="size-3.5" /></button></div></div>
          <div className="min-h-0 flex-1 overflow-y-auto p-2.5" style={{ scrollbarColor: '#3f3f46 transparent', scrollbarWidth: 'thin' }}><Accordion.Root type="multiple" value={openSections} onValueChange={setOpenSections} className="space-y-2">
              <ToolSection value="basic" title="Light & Color" subtitle="Core tonal response and chromatic balance" icon={Sun} activeCount={basicActive}><StudioSlider label="Brightness" code="LUMA" icon={Sun} value={settings.brightness} defaultValue={0} min={-100} max={100} description="Global luminance offset" onChange={(value) => updateSetting('brightness', value)} /><StudioSlider label="Contrast" code="CTR" icon={Contrast} value={settings.contrast} defaultValue={0} min={-100} max={100} description="Expand or compress tonal separation" onChange={(value) => updateSetting('contrast', value)} /><StudioSlider label="Saturation" code="SAT" icon={Droplets} value={settings.saturation} defaultValue={0} min={-100} max={100} description="Global chroma density" onChange={(value) => updateSetting('saturation', value)} /><StudioSlider label="Warmth" code="WB" icon={Thermometer} value={settings.warmth} defaultValue={0} min={-100} max={100} description="Shift white balance toward warm or cool" onChange={(value) => updateSetting('warmth', value)} /><StudioSlider label="Ambiance" code="AMB" icon={Sparkles} value={settings.ambiance} defaultValue={0} min={-100} max={100} description="Atmospheric lift for the global image" onChange={(value) => updateSetting('ambiance', value)} /><StudioSlider label="Highlights" code="HI" icon={Highlighter} value={settings.highlights} defaultValue={0} min={-100} max={100} description="Recover or accent bright regions" onChange={(value) => updateSetting('highlights', value)} /><StudioSlider label="Shadows" code="SH" icon={Layers2} value={settings.shadows} defaultValue={0} min={-100} max={100} description="Open or deepen dark regions" onChange={(value) => updateSetting('shadows', value)} /><div className="flex justify-end pt-1"><SectionReset onClick={resetBasic} /></div></ToolSection>
              <ToolSection value="fx" title="FX & Focus" subtitle="Blur, bokeh, curves and exposure compositing" icon={Aperture} activeCount={fxActive}><StudioSlider label="Global Blur" code="BLR" icon={Aperture} value={settings.blurRadius} defaultValue={0} min={0} max={40} description="Fast preview blur before advanced pass" unit="px" onChange={(value) => updateSetting('blurRadius', value)} /><StudioSlider label="Lens Blur" code="LENS" icon={Focus} value={advanced.lensBlur} defaultValue={0} min={0} max={40} description="Depth-inspired blur radius" unit="px" onChange={(value) => updateAdvanced('lensBlur', value)} /><StudioSlider label="Bokeh / Focus Shift" code="FOCUS" icon={Focus} value={advanced.bokeh} defaultValue={0} min={-100} max={100} description="Move the sharp focal band vertically" onChange={(value) => updateAdvanced('bokeh', value)} /><div className="space-y-2 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3"><div className="flex items-start justify-between gap-2"><div><div className="text-[13px] font-medium text-zinc-100">Curves</div><div className="mt-1 font-mono text-[9px] uppercase tracking-[0.1em] text-zinc-600">LUT • RGB master</div></div><span className="rounded-md border border-indigo-400/20 bg-indigo-500/10 px-2 py-1 font-mono text-[11px] text-indigo-200">{curveStrength > 0 ? '+' : ''}{curveStrength}</span></div><CurveMiniPreview y={curveStrength / 200} /><StudioSlider label="Curves Strength" code="LUT" icon={Sparkles} value={curveStrength} defaultValue={0} min={-100} max={100} description="Lift or compress the master midtone curve" onChange={(value) => { const v = value / 200; updateAdvanced('curves', [{ x: 0, y: 0 }, { x: 0.25, y: Math.max(0, 0.25 + v) }, { x: 0.5, y: Math.max(0, Math.min(1, 0.5 + v)) }, { x: 0.75, y: Math.max(0, Math.min(1, 0.75 + v)) }, { x: 1, y: 1 }]); }} /></div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg border border-white/[0.07] bg-zinc-950 text-zinc-400"><Layers2 className="size-4" /></span><div><div className="text-[12px] font-medium text-zinc-100">Double Exposure</div><div className="text-[10px] text-zinc-500">Secondary layer / blend pipeline</div></div></div></div><input type="file" accept="image/*" aria-label="Double Exposure file" onChange={(event) => { const file = event.target.files?.[0]; if (file) openDoubleExposure(file); event.currentTarget.value = ''; }} className="mt-3 block w-full cursor-pointer rounded-lg border border-white/[0.08] bg-zinc-950/80 px-3 py-2 text-[10px] text-zinc-500 file:mr-2 file:rounded file:border-0 file:bg-zinc-800 file:px-2 file:py-1 file:text-[10px] file:text-zinc-300" /><StudioSlider label="Exposure Opacity" code="EXP" icon={Blend} value={advanced.doubleExposureOpacity} defaultValue={0} min={0} max={100} description="Secondary layer strength" onChange={(value) => updateAdvanced('doubleExposureOpacity', value)} /><div className="mt-2"><select value={advanced.doubleExposureBlend} onChange={(event) => updateAdvanced('doubleExposureBlend', event.target.value as GlobalCompositeOperation)} aria-label="Exposure blend mode" className="w-full rounded-lg border border-white/[0.07] bg-zinc-950 px-3 py-2 text-xs text-zinc-300">{blendModes.map((mode) => <option key={mode} value={mode}>{mode}</option>)}</select></div></div><div className="flex justify-end pt-1"><SectionReset onClick={resetFx} /></div></ToolSection>
              <ToolSection value="geometry" title="Geometry" subtitle="Perspective and crop preparation" icon={MoveHorizontal} activeCount={geometryActive}><StudioSlider label="Perspective X" code="PX" icon={MoveHorizontal} value={advanced.perspectiveX} defaultValue={0} min={-50} max={50} description="Horizontal perspective correction" onChange={(value) => updateAdvanced('perspectiveX', value)} /><StudioSlider label="Perspective Y" code="PY" icon={MoveVertical} value={advanced.perspectiveY} defaultValue={0} min={-50} max={50} description="Vertical perspective correction" onChange={(value) => updateAdvanced('perspectiveY', value)} /><NumericField label="Crop X" value={settings.crop?.x ?? 0} defaultValue={0} onChange={(value) => updateSetting('crop', settings.crop ? { ...settings.crop, x: value } : { x: value, y: 0, width: image?.naturalWidth ?? 1, height: image?.naturalHeight ?? 1 })} /><NumericField label="Crop Y" value={settings.crop?.y ?? 0} defaultValue={0} onChange={(value) => updateSetting('crop', settings.crop ? { ...settings.crop, y: value } : { x: 0, y: value, width: image?.naturalWidth ?? 1, height: image?.naturalHeight ?? 1 })} /><div className="flex justify-end pt-1"><SectionReset onClick={resetGeometry} /></div></ToolSection>
              <ToolSection value="retouch" title="Retouch" subtitle="Selective brush and healing controls" icon={Eraser} activeCount={retouchActive}><StudioSlider label="Selective / Brush Strength" code="BRUSH" icon={Eraser} value={advanced.brushStrength} defaultValue={0} min={0} max={100} description="Paint to target localized regions" onChange={(value) => updateAdvanced('brushStrength', value)} /><NumericField label="Healing X" value={heal.x} defaultValue={0} min={0} max={image?.naturalWidth ?? 1} onChange={(value) => updateAdvanced('heal', { ...heal, x: value })} /><NumericField label="Healing Y" value={heal.y} defaultValue={0} min={0} max={image?.naturalHeight ?? 1} onChange={(value) => updateAdvanced('heal', { ...heal, y: value })} /><div className="flex justify-end pt-1"><SectionReset onClick={resetRetouch} /></div></ToolSection>
            </Accordion.Root></div>
        </aside>
      </div>
