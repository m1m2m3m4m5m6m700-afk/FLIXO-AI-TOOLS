    ...
    <div className="mx-auto flex min-h-[760px] w-full max-w-[1500px] flex-col gap-3 p-2 sm:p-3 lg:p-4">
      ...
      <main className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-zinc-950/90 shadow-[0_24px_70px_rgba(0,0,0,0.28)] backdrop-blur-xl">
          ...
          <div className="shrink-0 border-t border-white/[0.06] bg-zinc-950/95 p-2.5"><div className="grid grid-cols-3 gap-1.5"><button type="button" onClick={undo} disabled={historyIndex === 0} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-white/[0.06] bg-zinc-900 text-xs text-zinc-400 transition hover:text-white disabled:opacity-30"><Undo2 className="size-3.5" />Undo</button><button type="button" onClick={redo} disabled={historyIndex >= history.length - 1} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-white/[0.06] bg-zinc-900 text-xs text-zinc-400 transition hover:text-white disabled:opacity-30"><Redo2 className="size-3.5" />Redo</button><button type="button" onClick={exportImage} disabled={!image} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-indigo-500 text-xs font-semibold text-white shadow-[0_8px_24px_rgba(99,102,241,0.22)] transition hover:bg-indigo-400 disabled:opacity-40"><Download className="size-3.5" />Export</button></div></div>
        </aside>
      </main>
    </div>
  );
