...
type UiCopy = {
  description: string; choose: string; watermark: string; top: string; bottom: string;
  brightness: string; contrast: string; saturation: string; grayscale: string;
  processing: string; run: string; result: string; download: string; noResult: string;
  chooseImage: string; toolResult: string; alertOperationFailed: string;
};
...
type EffectCopy = Pick<UiCopy, 'brightness' | 'contrast' | 'saturation' | 'grayscale'>;
const EFFECT_COPY: Record<string, EffectCopy> = {
  ar: { brightness: 'السطوع', contrast: 'التباين', saturation: 'التشبع', grayscale: 'تدرج رمادي' },
  en: { brightness: 'Brightness', contrast: 'Contrast', saturation: 'Saturation', grayscale: 'Grayscale' },
  es: { brightness: 'Brillo', contrast: 'Contraste', saturation: 'Saturación', grayscale: 'Escala de grises' },
  fr: { brightness: 'Luminosité', contrast: 'Contraste', saturation: 'Saturation', grayscale: 'Niveaux de gris' },
  de: { brightness: 'Helligkeit', contrast: 'Kontrast', saturation: 'Sättigung', grayscale: 'Graustufen' },
  hi: { brightness: 'चमक', contrast: 'कंट्रास्ट', saturation: 'संतृप्ति', grayscale: 'ग्रेस्केल' },
  id: { brightness: 'Kecerahan', contrast: 'Kontras', saturation: 'Saturasi', grayscale: 'Skala abu-abu' },
  it: { brightness: 'Luminosità', contrast: 'Contrasto', saturation: 'Saturazione', grayscale: 'Scala di grigi' },
  ja: { brightness: '明るさ', contrast: 'コントラスト', saturation: '彩度', grayscale: 'グレースケール' },
  ko: { brightness: '밝기', contrast: '대비', saturation: '채도', grayscale: '회색조' },
  ms: { brightness: 'Kecerahan', contrast: 'Kontras', saturation: 'Ketepuan', grayscale: 'Skala kelabu' },
  nl: { brightness: 'Helderheid', contrast: 'Contrast', saturation: 'Verzadiging', grayscale: 'Grijstinten' },
  pl: { brightness: 'Jasność', contrast: 'Kontrast', saturation: 'Nasycenie', grayscale: 'Skala szarości' },
  pt: { brightness: 'Brilho', contrast: 'Contraste', saturation: 'Saturação', grayscale: 'Tons de cinza' },
  ru: { brightness: 'Яркость', contrast: 'Контраст', saturation: 'Насыщенность', grayscale: 'Оттенки серого' },
  sv: { brightness: 'Ljusstyrka', contrast: 'Kontrast', saturation: 'Mättnad', grayscale: 'Gråskala' },
  th: { brightness: 'ความสว่าง', contrast: 'คอนทราสต์', saturation: 'ความอิ่มตัว', grayscale: 'ระดับสีเทา' },
  tr: { brightness: 'Parlaklık', contrast: 'Kontrast', saturation: 'Doygunluk', grayscale: 'Gri tonlama' },
  uk: { brightness: 'Яскравість', contrast: 'Контраст', saturation: 'Насиченість', grayscale: 'Відтінки сірого' },
  vi: { brightness: 'Độ sáng', contrast: 'Độ tương phản', saturation: 'Độ bão hòa', grayscale: 'Thang độ xám' },
};
...
export function BrowserImageTool({ mode, title, accept = 'image/*', multi = false, locale }: Props) {
  ...
  const pageLocale = typeof document !== 'undefined' ? document.documentElement.lang.toLowerCase().split('-')[0] : resolvedLocale;
  const copy = UI_COPY[resolvedLocale];
  const effectCopy = EFFECT_COPY[pageLocale] ?? EFFECT_COPY[resolvedLocale];
  ...
  return <main dir={dir} lang={resolvedLocale} className="mx-auto max-w-3xl px-6 py-10">
    <h1 className="text-3xl font-bold">{title}</h1><p className="mt-2 text-sm opacity-70">{copy.description}</p>
    ...
    {mode === 'image-effects' && <div className="mt-4 grid gap-2 sm:grid-cols-2"><label>{effectCopy.brightness} <input aria-label={effectCopy.brightness} type="range" min="50" max="150" value={effect.brightness} onChange={(e) => setEffect({ ...effect, brightness: Number(e.target.value) })} /></label><label>{effectCopy.contrast} <input aria-label={effectCopy.contrast} type="range" min="50" max="150" value={effect.contrast} onChange={(e) => setEffect({ ...effect, contrast: Number(e.target.value) })} /></label><label>{effectCopy.saturation} <input aria-label={effectCopy.saturation} type="range" min="0" max="200" value={effect.saturate} onChange={(e) => setEffect({ ...effect, saturation: Number(e.target.value), saturate: Number(e.target.value) })} /></label><label>{effectCopy.grayscale} <input aria-label={effectCopy.grayscale} type="range" min="0" max="100" value={effect.grayscale} onChange={(e) => setEffect({ ...effect, grayscale: Number(e.target.value) })} /></label></div>}
