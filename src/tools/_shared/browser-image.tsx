...
export function BrowserImageTool({ mode, title, accept = 'image/*', multi = false, locale }: Props) {
  ...
  return <main dir={dir} lang={resolvedLocale} className="mx-auto max-w-3xl px-6 py-10">
    ...
    {mode === 'image-effects' && <div className="mt-4 grid gap-2 sm:grid-cols-2"><label>{effectCopy.brightness} <input aria-label={effectCopy.brightness} type="range" min="50" max="150" value={effect.brightness} onChange={(e) => setEffect({ ...effect, brightness: Number(e.target.value) })} /></label><label>{effectCopy.contrast} <input aria-label={effectCopy.contrast} type="range" min="50" max="150" value={effect.contrast} onChange={(e) => setEffect({ ...effect, contrast: Number(e.target.value) })} /></label><label>{effectCopy.saturation} <input aria-label={effectCopy.saturation} type="range" min="0" max="200" value={effect.saturate} onChange={(e) => setEffect({ ...effect, saturate: Number(e.target.value) })} /></label><label>{effectCopy.grayscale} <input aria-label={effectCopy.grayscale} type="range" min="0" max="100" value={effect.grayscale} onChange={(e) => setEffect({ ...effect, grayscale: Number(e.target.value) })} /></label></div>}
