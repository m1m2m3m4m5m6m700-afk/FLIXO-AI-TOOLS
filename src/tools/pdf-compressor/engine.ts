...
const DEFAULT_OPTIONS: PdfCompressionOptions = { level: 'medium', jpegQuality: 0.62, renderScale: 1.5, maxPagePixels: 12_000_000 };
const LEVELS: Record<PdfCompressionLevel, Pick<PdfCompressionOptions, 'jpegQuality' | 'renderScale'>> = {
  low: { jpegQuality: 0.78, renderScale: 2 }, medium: { jpegQuality: 0.62, renderScale: 1.5 }, high: { jpegQuality: 0.44, renderScale: 1.05 },
};
...
function canvasToJpeg(canvas: HTMLCanvasElement, quality: number) {
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((blob) => { if (!blob) reject(new Error('PDF page image encoding failed.')); else resolve(blob); }, 'image/jpeg', clamp(quality, 0.15, 0.92));
  });
}
...
async function buildCompressedPdf(pdf: PDFDocumentProxy, options: PdfCompressionOptions) {
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    const image = await output.embedJpg(await rendered.jpeg.arrayBuffer());
    const targetPage = output.addPage([rendered.widthPoints, rendered.heightPoints]);
    targetPage.drawImage(image, { x: 0, y: 0, width: rendered.widthPoints, height: rendered.heightPoints });
    page.cleanup();
  }
...
export async function compressPdf(file: File, incomingOptions?: Partial<PdfCompressionOptions>): Promise<PdfCompressionResult> {
  if (file.type && file.type !== 'application/pdf' && !file.name.toLowerCase().endsWith('.pdf')) throw new Error('Please select a PDF file.');
  ...
  const options: PdfCompressionOptions = {
    ...DEFAULT_OPTIONS, ...preset, ...incomingOptions,
  ...
  const savingsPercent = usedCompression ? Math.max(0, Math.round((1 - outputBytesCount / file.size) * 100)) : 0;
  const buffer = new ArrayBuffer(finalBytes.byteLength); new Uint8Array(buffer).set(finalBytes);
  return { blob: new Blob([buffer], { type: 'application/pdf' }), inputBytes: file.size, outputBytes: outputBytesCount, savingsPercent, pageCount: pdf.numPages, usedCompression };
}
