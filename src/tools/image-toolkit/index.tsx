...
type UiCopy = Readonly<{
  imageTools: string; prompt: string; chooseImage: string; chooseImageFirst: string; imageInput: string;
  outputFormat: string; scale: string; backgroundTolerance: string; svgColumns: string;
  width: string; height: string; outputWidth: string; outputHeight: string;
...
const UI_COPY: Record<string, UiCopy> = {
  en: { imageTools:'FLIXO · IMAGE TOOLS', prompt:'Prompt', chooseImage:'Choose an image', chooseImageFirst:'Choose an image first.', imageInput:'IMAGE INPUT', outputFormat:'Output format', scale:'Scale', backgroundTolerance:'Background tolerance', svgColumns:'SVG columns', width:'Width', height:'Height', outputWidth:'Output width', outputHeight:'Output height', run:'Run tool', processing:'Processing…', generate:'Generate image', result:'RESULT', download:'Download', downloadNow:'Download now', noResult:'No result yet.', toolResult:'Tool result', privacyOcr:'OCR preprocesses the selected image locally, then runs Tesseract.js recognition in a dedicated Web Worker.' },
  ar: { imageTools:'FLIXO · أدوات الصور', prompt:'الوصف', chooseImage:'اختر صورة', chooseImageFirst:'اختر صورة أولًا.', imageInput:'مدخل الصورة', outputFormat:'تنسيق الإخراج', scale:'المقياس', backgroundTolerance:'تسامح الخلفية', svgColumns:'أعمدة SVG', width:'العرض', height:'الارتفاع', outputWidth:'عرض الإخراج', outputHeight:'ارتفاع الإخراج', run:'تشغيل الأداة', processing:'جارٍ المعالجة…', generate:'إنشاء صورة', result:'النتيجة', download:'تنزيل', downloadNow:'تنزيل الآن', noResult:'لا توجد نتيجة بعد.', toolResult:'نتيجة الأداة', privacyOcr:'تُعالج الصورة المحددة محليًا أولًا، ثم يتعرف Tesseract.js على النص داخل Web Worker مخصص.' },
  es: { imageTools:'FLIXO · HERRAMIENTAS DE IMAGEN', prompt:'Indicación', chooseImage:'Elige una imagen', chooseImageFirst:'Elige una imagen primero.', imageInput:'ENTRADA DE IMAGEN', outputFormat:'Formato de salida', scale:'Escala', backgroundTolerance:'Tolerancia del fondo', svgColumns:'Columnas SVG', width:'Ancho', height:'Alto', outputWidth:'Ancho de salida', outputHeight:'Alto de salida', run:'Ejecutar herramienta', processing:'Procesando…', generate:'Generar imagen', result:'RESULTADO', download:'Descargar', downloadNow:'Descargar ahora', noResult:'Aún no hay resultado.', toolResult:'Resultado de la herramienta', privacyOcr:'La imagen seleccionada se preprocesa localmente y Tesseract.js reconoce el texto en un Web Worker dedicado.' },
  fr: { imageTools:'FLIXO · OUTILS D’IMAGE', prompt:'Consigne', chooseImage:'Choisissez une image', chooseImageFirst:'Choisissez d’abord une image.', imageInput:'ENTRÉE IMAGE', outputFormat:'Format de sortie', scale:'Échelle', backgroundTolerance:'Tolérance de l’arrière-plan', svgColumns:'Colonnes SVG', width:'Largeur', height:'Hauteur', outputWidth:'Largeur de sortie', outputHeight:'Hauteur de sortie', run:'Exécuter l’outil', processing:'Traitement…', generate:'Générer une image', result:'RÉSULTAT', download:'Télécharger', downloadNow:'Télécharger maintenant', noResult:'Aucun résultat pour le moment.', toolResult:'Résultat de l’outil', privacyOcr:'L’image sélectionnée est prétraitée localement, puis Tesseract.js reconnaît le texte dans un Web Worker dédié.' },
  de: { imageTools:'FLIXO · BILDTOOLS', prompt:'Eingabe', chooseImage:'Bild auswählen', chooseImageFirst:'Wählen Sie zuerst ein Bild aus.', imageInput:'BILDEINGABE', outputFormat:'Ausgabeformat', scale:'Skalierung', backgroundTolerance:'Hintergrundtoleranz', svgColumns:'SVG-Spalten', width:'Breite', height:'Höhe', outputWidth:'Ausgabebreite', outputHeight:'Ausgabehöhe', run:'Tool ausführen', processing:'Verarbeitung…', generate:'Bild generieren', result:'ERGEBNIS', download:'Herunterladen', downloadNow:'Jetzt herunterladen', noResult:'Noch kein Ergebnis.', toolResult:'Tool-Ergebnis', privacyOcr:'Das ausgewählte Bild wird lokal vorverarbeitet; anschließend erkennt Tesseract.js den Text in einem dedizierten Web Worker.' },
  hi: { imageTools:'FLIXO · इमेज टूल्स', prompt:'प्रॉम्प्ट', chooseImage:'छवि चुनें', chooseImageFirst:'पहले एक छवि चुनें।', imageInput:'इमेज इनपुट', outputFormat:'आउटपुट फ़ॉर्मेट', scale:'स्केल', backgroundTolerance:'बैकग्राउंड टॉलरेंस', svgColumns:'SVG कॉलम', width:'चौड़ाई', height:'ऊँचाई', outputWidth:'आउटपुट चौड़ाई', outputHeight:'आउटपुट ऊँचाई', run:'टूल चलाएँ', processing:'प्रोसेस हो रहा है…', generate:'छवि बनाएँ', result:'परिणाम', download:'डाउनलोड', downloadNow:'अभी डाउनलोड करें', noResult:'अभी कोई परिणाम नहीं।', toolResult:'टूल परिणाम', privacyOcr:'चयनित छवि को स्थानीय रूप से प्रीप्रोसेस किया जाता है और फिर समर्पित Web Worker में Tesseract.js पहचान चलती है।' },
  id: { imageTools:'FLIXO · ALAT GAMBAR', prompt:'Perintah', chooseImage:'Pilih gambar', chooseImageFirst:'Pilih gambar terlebih dahulu.', imageInput:'MASUKAN GAMBAR', outputFormat:'Format keluaran', scale:'Skala', backgroundTolerance:'Toleransi latar belakang', svgColumns:'Kolom SVG', width:'Lebar', height:'Tinggi', outputWidth:'Lebar keluaran', outputHeight:'Tinggi keluaran', run:'Jalankan alat', processing:'Memproses…', generate:'Buat gambar', result:'HASIL', download:'Unduh', downloadNow:'Unduh sekarang', noResult:'Belum ada hasil.', toolResult:'Hasil alat', privacyOcr:'Gambar pilihan diproses secara lokal, lalu Tesseract.js mengenali teks dalam Web Worker khusus.' },
  it: { imageTools:'FLIXO · STRUMENTI IMMAGINE', prompt:'Prompt', chooseImage:'Scegli un’immagine', chooseImageFirst:'Scegli prima un’immagine.', imageInput:'INPUT IMMAGINE', outputFormat:'Formato di output', scale:'Scala', backgroundTolerance:'Tolleranza dello sfondo', svgColumns:'Colonne SVG', width:'Larghezza', height:'Altezza', outputWidth:'Larghezza di output', outputHeight:'Altezza di output', run:'Esegui lo strumento', processing:'Elaborazione…', generate:'Genera immagine', result:'RISULTATO', download:'Scarica', downloadNow:'Scarica ora', noResult:'Nessun risultato ancora.', toolResult:'Risultato dello strumento', privacyOcr:'L’immagine selezionata viene preelaborata localmente, quindi Tesseract.js riconosce il testo in un Web Worker dedicato.' },
  ja: { imageTools:'FLIXO · 画像ツール', prompt:'プロンプト', chooseImage:'画像を選択', chooseImageFirst:'最初に画像を選択してください。', imageInput:'画像入力', outputFormat:'出力形式', scale:'倍率', backgroundTolerance:'背景の許容値', svgColumns:'SVG列数', width:'幅', height:'高さ', outputWidth:'出力幅', outputHeight:'出力高さ', run:'ツールを実行', processing:'処理中…', generate:'画像を生成', result:'結果', download:'ダウンロード', downloadNow:'今すぐダウンロード', noResult:'まだ結果はありません。', toolResult:'ツールの結果', privacyOcr:'選択した画像をローカルで前処理し、専用のWeb WorkerでTesseract.jsによる認識を実行します。' },
  ko: { imageTools:'FLIXO · 이미지 도구', prompt:'프롬프트', chooseImage:'이미지 선택', chooseImageFirst:'먼저 이미지를 선택하세요.', imageInput:'이미지 입력', outputFormat:'출력 형식', scale:'배율', backgroundTolerance:'배경 허용 범위', svgColumns:'SVG 열', width:'너비', height:'높이', outputWidth:'출력 너비', outputHeight:'출력 높이', run:'도구 실행', processing:'처리 중…', generate:'이미지 생성', result:'결과', download:'다운로드', downloadNow:'지금 다운로드', noResult:'아직 결과가 없습니다.', toolResult:'도구 결과', privacyOcr:'선택한 이미지를 로컬에서 전처리한 다음 전용 Web Worker에서 Tesseract.js 인식을 실행합니다.' },
  ms: { imageTools:'FLIXO · ALAT IMEJ', prompt:'Gesaan', chooseImage:'Pilih imej', chooseImageFirst:'Pilih imej dahulu.', imageInput:'INPUT IMEJ', outputFormat:'Format output', scale:'Skala', backgroundTolerance:'Toleransi latar', svgColumns:'Lajur SVG', width:'Lebar', height:'Tinggi', outputWidth:'Lebar output', outputHeight:'Tinggi output', run:'Jalankan alat', processing:'Memproses…', generate:'Jana imej', result:'HASIL', download:'Muat turun', downloadNow:'Muat turun sekarang', noResult:'Tiada hasil lagi.', toolResult:'Hasil alat', privacyOcr:'Imej yang dipilih dipraproses secara setempat, kemudian Tesseract.js menjalankan pengecaman dalam Web Worker khusus.' },
  nl: { imageTools:'FLIXO · AFBEELDINGSTOOLS', prompt:'Prompt', chooseImage:'Kies een afbeelding', chooseImageFirst:'Kies eerst een afbeelding.', imageInput:'AFBEELDINGSINVOER', outputFormat:'Uitvoerindeling', scale:'Schaal', backgroundTolerance:'Achtergrondtolerantie', svgColumns:'SVG-kolommen', width:'Breedte', height:'Hoogte', outputWidth:'Uitvoerbreedte', outputHeight:'Uitvoerhoogte', run:'Tool uitvoeren', processing:'Bezig met verwerken…', generate:'Afbeelding genereren', result:'RESULTAAT', download:'Downloaden', downloadNow:'Nu downloaden', noResult:'Nog geen resultaat.', toolResult:'Toolresultaat', privacyOcr:'De geselecteerde afbeelding wordt lokaal voorbewerkt en Tesseract.js voert herkenning uit in een speciale Web Worker.' },
  pl: { imageTools:'FLIXO · NARZĘDZIA DO OBRAZÓW', prompt:'Polecenie', chooseImage:'Wybierz obraz', chooseImageFirst:'Najpierw wybierz obraz.', imageInput:'WEJŚCIE OBRAZU', outputFormat:'Format wyjściowy', scale:'Skala', backgroundTolerance:'Tolerancja tła', svgColumns:'Kolumny SVG', width:'Szerokość', height:'Wysokość', outputWidth:'Szerokość wyjścia', outputHeight:'Wysokość wyjścia', run:'Uruchom narzędzie', processing:'Przetwarzanie…', generate:'Wygeneruj obraz', result:'WYNIK', download:'Pobierz', downloadNow:'Pobierz teraz', noResult:'Brak wyniku.', toolResult:'Wynik narzędzia', privacyOcr:'Wybrany obraz jest przetwarzany lokalnie, a następnie Tesseract.js rozpoznaje tekst w dedykowanym Web Workerze.' },
  pt: { imageTools:'FLIXO · FERRAMENTAS DE IMAGEM', prompt:'Comando', chooseImage:'Escolha uma imagem', chooseImageFirst:'Escolha primeiro uma imagem.', imageInput:'ENTRADA DE IMAGEM', outputFormat:'Formato de saída', scale:'Escala', backgroundTolerance:'Tolerância do fundo', svgColumns:'Colunas SVG', width:'Largura', height:'Altura', outputWidth:'Largura de saída', outputHeight:'Altura de saída', run:'Executar ferramenta', processing:'A processar…', generate:'Gerar imagem', result:'RESULTADO', download:'Transferir', downloadNow:'Transferir agora', noResult:'Ainda não há resultado.', toolResult:'Resultado da ferramenta', privacyOcr:'A imagem selecionada é pré-processada localmente e o Tesseract.js reconhece o texto num Web Worker dedicado.' },
  ru: { imageTools:'FLIXO · ИНСТРУМЕНТЫ ДЛЯ ИЗОБРАЖЕНИЙ', prompt:'Запрос', chooseImage:'Выберите изображение', chooseImageFirst:'Сначала выберите изображение.', imageInput:'ВХОДНОЕ ИЗОБРАЖЕНИЕ', outputFormat:'Формат вывода', scale:'Масштаб', backgroundTolerance:'Допуск фона', svgColumns:'Столбцы SVG', width:'Ширина', height:'Высота', outputWidth:'Ширина результата', outputHeight:'Высота результата', run:'Запустить инструмент', processing:'Обработка…', generate:'Создать изображение', result:'РЕЗУЛЬТАТ', download:'Скачать', downloadNow:'Скачать сейчас', noResult:'Результата пока нет.', toolResult:'Результат инструмента', privacyOcr:'Выбранное изображение обрабатывается локально, затем Tesseract.js распознаёт текст в выделенном Web Worker.' },
  sv: { imageTools:'FLIXO · BILDVERKTYG', prompt:'Prompt', chooseImage:'Välj en bild', chooseImageFirst:'Välj en bild först.', imageInput:'BILDINDATA', outputFormat:'Utdataformat', scale:'Skala', backgroundTolerance:'Bakgrundstolerans', svgColumns:'SVG-kolumner', width:'Bredd', height:'Höjd', outputWidth:'Utdata-bredd', outputHeight:'Utdata-höjd', run:'Kör verktyget', processing:'Bearbetar…', generate:'Skapa bild', result:'RESULTAT', download:'Ladda ner', downloadNow:'Ladda ner nu', noResult:'Inget resultat ännu.', toolResult:'Verktygsresultat', privacyOcr:'Den valda bilden förbehandlas lokalt och Tesseract.js kör igenkänning i en dedikerad Web Worker.' },
  th: { imageTools:'FLIXO · เครื่องมือรูปภาพ', prompt:'พรอมต์', chooseImage:'เลือกภาพ', chooseImageFirst:'เลือกภาพก่อน', imageInput:'อินพุตรูปภาพ', outputFormat:'รูปแบบเอาต์พุต', scale:'มาตราส่วน', backgroundTolerance:'ค่าความคลาดเคลื่อนของพื้นหลัง', svgColumns:'คอลัมน์ SVG', width:'ความกว้าง', height:'ความสูง', outputWidth:'ความกว้างเอาต์พุต', outputHeight:'ความสูงเอาต์พุต', run:'เรียกใช้เครื่องมือ', processing:'กำลังประมวลผล…', generate:'สร้างภาพ', result:'ผลลัพธ์', download:'ดาวน์โหลด', downloadNow:'ดาวน์โหลดตอนนี้', noResult:'ยังไม่มีผลลัพธ์', toolResult:'ผลลัพธ์ของเครื่องมือ', privacyOcr:'ภาพที่เลือกจะถูกประมวลผลล่วงหน้าในเครื่อง จากนั้น Tesseract.js จะรู้จำข้อความใน Web Worker เฉพาะ' },
  tr: { imageTools:'FLIXO · GÖRSEL ARAÇLARI', prompt:'İstek', chooseImage:'Bir görsel seçin', chooseImageFirst:'Önce bir görsel seçin.', imageInput:'GÖRSEL GİRDİSİ', outputFormat:'Çıktı biçimi', scale:'Ölçek', backgroundTolerance:'Arka plan toleransı', svgColumns:'SVG sütunları', width:'Genişlik', height:'Yükseklik', outputWidth:'Çıktı genişliği', outputHeight:'Çıktı yüksekliği', run:'Aracı çalıştır', processing:'İşleniyor…', generate:'Görsel oluştur', result:'SONUÇ', download:'İndir', downloadNow:'Şimdi indir', noResult:'Henüz sonuç yok.', toolResult:'Araç sonucu', privacyOcr:'Seçilen görsel yerel olarak ön işlenir, ardından Tesseract.js ayrılmış bir Web Worker içinde tanıma yapar.' },
  uk: { imageTools:'FLIXO · ІНСТРУМЕНТИ ЗОБРАЖЕНЬ', prompt:'Запит', chooseImage:'Виберіть зображення', chooseImageFirst:'Спочатку виберіть зображення.', imageInput:'ВХІДНЕ ЗОБРАЖЕННЯ', outputFormat:'Формат виводу', scale:'Масштаб', backgroundTolerance:'Допуск фону', svgColumns:'Стовпці SVG', width:'Ширина', height:'Висота', outputWidth:'Ширина виводу', outputHeight:'Висота виводу', run:'Запустити інструмент', processing:'Обробка…', generate:'Створити зображення', result:'РЕЗУЛЬТАТ', download:'Завантажити', downloadNow:'Завантажити зараз', noResult:'Результату ще немає.', toolResult:'Результат інструмента', privacyOcr:'Вибране зображення обробляється локально, після чого Tesseract.js розпізнає текст у спеціальному Web Worker.' },
  vi: { imageTools:'FLIXO · CÔNG CỤ HÌNH ẢNH', prompt:'Lời nhắc', chooseImage:'Chọn ảnh', chooseImageFirst:'Trước tiên hãy chọn một ảnh.', imageInput:'ĐẦU VÀO HÌNH ẢNH', outputFormat:'Định dạng đầu ra', scale:'Tỷ lệ', backgroundTolerance:'Dung sai nền', svgColumns:'Cột SVG', width:'Chiều rộng', height:'Chiều cao', outputWidth:'Chiều rộng đầu ra', outputHeight:'Chiều cao đầu ra', run:'Chạy công cụ', processing:'Đang xử lý…', generate:'Tạo ảnh', result:'KẾT QUẢ', download:'Tải xuống', downloadNow:'Tải xuống ngay', noResult:'Chưa có kết quả.', toolResult:'Kết quả công cụ', privacyOcr:'Ảnh đã chọn được xử lý trước cục bộ, sau đó Tesseract.js nhận dạng văn bản trong Web Worker chuyên dụng.' },
};
      ...
      <div className="image-tool-container">
        ...
        <section className="compressor-grid" aria-label={definition.title}>
          <div className="compressor-card">
            {isGenerator
              ? <label><span>{ui.prompt}</span><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={ui.prompt} rows={6} /></label>
            ...
            {['object-remover', 'watermark-remover', 'crop-resize'].includes(toolId) && <div className="control-grid">{([ [ui.width, cropX, setCropX], [ui.height, cropY, setCropY], [ui.width, cropW, setCropW], [ui.height, cropH, setCropH] ] as const).map(([labelText, value, setter], index) => <label key={`${labelText}-${index}`}><span>{labelText}</span><input aria-label={labelText} inputMode="numeric" value={value} onChange={(event) => setter(event.target.value)} /></label>)}{toolId === 'crop-resize' && <><label><span>{ui.outputWidth}</span><input aria-label={ui.outputWidth} inputMode="numeric" value={outW} onChange={(event) => setOutW(event.target.value)} /></label><label><span>{ui.outputHeight}</span><input aria-label={ui.outputHeight} inputMode="numeric" value={outH} onChange={(event) => setOutH(event.target.value)} /></label></>}</div>}
