...
export function ImageToolPage({ toolId }: Props) {
  ...
  const previewUrl = useObjectUrl(result?.blob.type.startsWith('image/') ? result.blob : null); const downloadUrl = useObjectUrl(result?.blob ?? null);
  ...
  const run = async () => {
    ...
    try {
      if (isGenerator) {
        ...
        const info = await imageInfo(blob); setResult({ blob, info, fileName: `flixo-ai-${info.width}x${info.height}.png` }); return;
      }
      ...
      else if (toolId === 'image-to-text') { const prepared = await preprocessForOcr(file); const ocr = await recognizeWithOcrWorker(prepared, 'eng+ara'); setResult({ blob: new Blob([ocr.text], { type: 'text/plain;charset=utf-8' }), text: ocr.text, fileName: `${baseName(file.name)}.txt` }); return; }
      ...
      <div className="image-tool-container">
        ...
        <section className="compressor-grid" aria-label={definition.title}>
          <div className="compressor-card">
            ...
            {toolId === 'image-converter' && <label><span>Output format</span><select value={outputFormat} onChange={(event) => setOutputFormat(event.target.value as typeof outputFormat)}><option value="image/webp">WebP</option><option value="image/jpeg">JPG</option><option value="image/png">PNG</option></select></label>}
            {toolId === 'image-upscaler' && <label><span>Scale</span><input inputMode="decimal" value={scale} onChange={(event) => setScale(event.target.value)} /></label>}
            {toolId === 'background-remover' && <label><span>Background tolerance</span><input inputMode="numeric" value={tolerance} onChange={(event) => setTolerance(event.target.value)} /></label>}
            {toolId === 'raster-to-svg' && <label><span>SVG columns</span><input inputMode="numeric" value={columns} onChange={(event) => setColumns(event.target.value)} /></label>}
            {['object-remover', 'watermark-remover', 'crop-resize'].includes(toolId) && <div className="control-grid">{([['X', cropX, setCropX], ['Y', cropY, setCropY], ['Width', cropW, setCropW], ['Height', cropH, setCropH]] as const).map(([labelText, value, setter]) => <label key={labelText}><span>{labelText}</span><input inputMode="numeric" value={value} onChange={(event) => setter(event.target.value)} /></label>)}{toolId === 'crop-resize' && <><label><span>Output width</span><input inputMode="numeric" value={outW} onChange={(event) => setOutW(event.target.value)} /></label><label><span>Output height</span><input inputMode="numeric" value={outH} onChange={(event) => setOutH(event.target.value)} /></label></>}</div>}
