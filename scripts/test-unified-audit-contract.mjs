...
const completeness = await read('src/lib/i18n/tool-ui-runtime-completeness.ts');
...
assert.match(completeness, /installScopedRuntimeObserver/u);
assert.doesNotMatch(completeness, /\.observe\(\s*document\.body\b/u);
assert.doesNotMatch(completeness, /document\.querySelector<HTMLElement>\('\.tool-page-modern, \.tool-shell, main'\)/u);
