...
for (const token of ['application/pdf', 'audio/', 'video/', '.mp3', '.mp4', '.csv']) {
  assert.equal(fileSafety.includes(token), false, `legacy G2 media format remains: ${token}`);
}
...
for (const token of ['PDFDocument', 'application/pdf', 'text/csv', 'audio/', 'video/']) {
  assert.equal(g3ExecutableSource.includes(token), false, `legacy G3 format remains: ${token}`);
}
