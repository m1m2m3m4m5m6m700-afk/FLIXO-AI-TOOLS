...
const run = (command, args = [], extraEnv = {}) => execFileSync(command, args, { cwd: root, stdio: 'inherit', env: { ...process.env, ...extraEnv } });
...
if (canonicalMaster.length < 4096) fail(`canonical FLIXO master artwork is unexpectedly small: ${canonicalMaster.length} bytes`);
if (canonicalMaster[0] !== 0xff || canonicalMaster[1] !== 0xd8 || canonicalMaster[2] !== 0xff) fail('canonical FLIXO master artwork is not a valid JPEG');
...
if (!canonicalLogo.includes('href="/flixo-logo.jpg"')) fail('canonical logo must render the exact master /flixo-logo.jpg');
...
if (!existsSync(builtRobotsPath)) fail('build output is missing dist/robots.txt');
if (!existsSync(builtSitemapPath)) fail('build output is missing dist/sitemap.xml');
const expectedRobots = `User-agent: *\nAllow: /\n\nSitemap: ${siteUrl}/sitemap.xml\n`;
if (readFileSync(builtRobotsPath, 'utf8') !== expectedRobots) fail('dist/robots.txt does not match the canonical origin contract');
pass('deployment metadata emitted into dist without mutating public sources');
...
try {
  ...
  const exactAllow = new Set([
    '.github/workflows/ci.yml', '.github/workflows/full-matrix-promotion.yml', '.github/workflows/root-cause-diagnostics.yml', '.github/workflows/s4-runtime-e2e.yml', '.github/workflows/localization-20.yml',
    'playwright.config.ts', 'scripts/validate-s3-static-gate.mjs', 'scripts/validate-s4-e2e.mjs', 'scripts/validate-site-origin.mjs', 'scripts/test-route-resolver.mjs', 'scripts/test-i18n-contract.mjs', 'scripts/test-seo-contract.mjs', 'scripts/test-upload-boundary.mjs',
    'scripts/validate-language-quality.mjs', 'scripts/validate-language-quality-strict.mjs', 'scripts/validate-locale-contract.mjs', 'scripts/validate-localization-complete.mjs', 'scripts/validation-utils.mjs', 'scripts/validate-indexing.mjs', 'scripts/validate-google-multilingual-seo.mjs',
    'scripts/node-resolver-loader.mjs', 'scripts/register-node-resolver.mjs', 'scripts/generate-robots.mjs', 'scripts/generate-sitemap.mjs', 'scripts/validate-router-registry.mjs', 'README.md', 'README', 'docs/CONSOLIDATION-LOG.md', 'docs/DEBT-REGISTER.md',
    ...
    'src/lib/routing/route-resolver.ts', 'src/lib/seo/tool-seo.ts', 'src/routes/__root.tsx', 'src/routes/home-page.tsx', 'src/routes/localized-home.tsx', 'src/routes/locale-pages.tsx', 'src/data/home-locales.ts', 'src/data/quickflow-locales.ts', 'src/data/tool-ui-i18n.ts', 'src/components/FlixoGlobalLogo.tsx', 'src/components/auto-localized-tool-surface.tsx', 'src/lib/contracts/upload-boundary.ts', 'src/tools/image-toolkit/index.tsx', 'src/tools/image-compressor/index.tsx', 'tests/image-converter.contract.spec.ts', 'tests/localization-runtime.spec.ts',
