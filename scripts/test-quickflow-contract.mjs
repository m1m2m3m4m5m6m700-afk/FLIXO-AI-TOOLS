...
const compress = buildQuickFlowPlan('compress image', TOOL_REGISTRY);
assert.equal(compress?.version, 2);
assert.deepEqual(compress?.steps.map((step) => step.toolId), ['image-compressor']);

const arabic = buildQuickFlowPlan('إزالة الخلفية', TOOL_REGISTRY);
assert.deepEqual(arabic?.steps.map((step) => step.toolId), ['background-remover']);

const productPreset = buildQuickFlowPlan('prepare product image for store', TOOL_REGISTRY);
...
const composite = buildQuickFlowPlan('جهز صورة المنتج للمتجر بأقل من 200KB وصيغة WebP', TOOL_REGISTRY);
...
const unknown = buildQuickFlowPlan('do something unrelated', TOOL_REGISTRY);
assert.equal(unknown, null);

const empty = buildQuickFlowPlan('   ', TOOL_REGISTRY);
assert.equal(empty, null);

const invalid = buildQuickFlowPlan('resize to 5000x5000', TOOL_REGISTRY);
assert.equal(invalid, null);

const notReadyTools = TOOL_REGISTRY.map((tool) => tool.id === 'image-compressor' ? { ...tool, isReady: false } : tool);
