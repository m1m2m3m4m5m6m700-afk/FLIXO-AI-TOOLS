...
const add = (gate, ok, detail, className = 'ENVIRONMENT', rootCause = gate) => {
  const record = {
    gate,
    status: ok ? 'PASS' : 'FAIL',
    class: ok ? null : className,
    rootCause: ok ? null : rootCause,
    retryable: false,
    sha: process.env.EXPECTED_HEAD_SHA || process.env.GITHUB_SHA || 'unknown',
    durationMs: Date.now() - started,
    command: `environment check ${gate}`,
    detail,
  };
  checks.push(record);
...
const runtime = {
  ...
  typescript: await run('npx', ['tsc', '--version']),
  vite: await run('npx', ['vite', '--version']),
  playwright: await run('npx', ['playwright', '--version']),
};
...
const report = {
  ...
  class: failures.length ? 'ENVIRONMENT' : null,
  rootCause: failures[0]?.rootCause ?? null,
  runtime,
  vars,
  checks,
  failures,
...
for (const check of checks) {
  await fs.writeFile(`artifacts/ci/g3/${check.gate.replaceAll('/', '-')}.json`, JSON.stringify(check, null, 2) + '\n');
}
