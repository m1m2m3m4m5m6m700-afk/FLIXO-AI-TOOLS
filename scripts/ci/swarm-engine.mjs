...
function emptyState() {
  return { claims: new Map(), workItems: new Map(), bids: new Map(), lastTimestamp: 0, lastHash: 'GENESIS' };
}
...
function activeClaims(state, at = Date.now()) {
  return [...state.claims.values()].filter((claim) => claim.status === 'active' && Date.parse(claim.leaseUntil) > at);
}
  ...
  for (const event of events) {
    ...
    if (event.type === 'STATE_BOOTSTRAP') {
      ...
      for (const patch of p.workItems ?? []) {
        const item = state.workItems.get(patch.id);
        if (!item) fail('SWARM_EVENT_STATE_CONFLICT', `reconcile references unknown work item ${patch.id}`);
      ...
      state.bids.set(p.workItemId, list);
    } else if (event.type === 'CLAIM_AWARDED') {
      const item = state.workItems.get(p.workItemId);
      if (!item) fail('SWARM_EVENT_STATE_CONFLICT', `unknown work item ${p.workItemId}`);
      ...
      const claim = state.claims.get(p.agentId);
      if (claim?.status === 'active') claim.workItemIds = unique([...(claim.workItemIds ?? []), p.workItemId]);
    } else if (event.type === 'EXPIRED_EVICTED') {
      const claim = state.claims.get(p.agentId);
      if (!claim) fail('SWARM_EVENT_STATE_CONFLICT', `cannot evict unknown agent ${p.agentId}`);
      ...
      if (workItemId) {
        ...
        if (!item) fail('SWARM_EVENT_STATE_CONFLICT', `unknown work item ${workItemId}`);
...
function projectionPayload(state) {
  ...
  return {
    claims,
    activeSessions: claims.filter((claim) => claim.status === 'active').map((claim) => ({
...
async function flushProjections(state) {
  ...
  const expectedClaims = stableProjection(claimsProjection);
  const expectedSessions = stableProjection(sessionsProjection);
  const priorClaims = stableProjection(await readJsonIfPresent(CLAIMS));
  const priorSessions = stableProjection(await readJsonIfPresent(SESSIONS));
  const desyncDetected = JSON.stringify(priorClaims) !== JSON.stringify(expectedClaims) || JSON.stringify(priorSessions) !== JSON.stringify(expectedSessions);
...
async function replayAndProject() {
  const events = await readEvents();
  const state = reduce(events);
  const projection = await flushProjections(state);
  return { events, state, projection };
}
async function sweepExpired(state, parentSha) {
  const expired = activeClaims(state).filter((claim) => {
    const leaseExpired = Date.now() > parseTime(claim.leaseUntil, `${claim.agentId}.leaseUntil`);
    const heartbeatAt = parseTime(claim.lastHeartbeatAt ?? claim.leasedAt, `${claim.agentId}.heartbeat`);
    return leaseExpired || Date.now() - heartbeatAt > HEARTBEAT_MS;
  });
  for (const claim of expired) {
    const events = await readEvents();
    const item = [...state.workItems.values()].find((workItem) => workItem.ownerAgentId === claim.agentId);
    ...
    Object.assign(state, reduce(latest));
  }
  return parentSha;
}
...
async function mutate(action) {
  ...
  let { state } = await replayAndProject();
  if (action === 'sweep') {
  ...
  if (action === 'check-in') {
    ...
    if (!scope.paths.length && !scope.contracts.length && !scope.rootCauseIds.length) fail('SWARM_OCC_CONFLICT', 'claim scope cannot be empty');
    const collisions = activeClaims(state).filter((claim) => claim.agentId !== AGENT && claimCollides(scope, claim));
    if (collisions.length) fail('AGENT_COLLISION_DETECTED', collisions.map((claim) => claim.agentId).sort().join(','));
    const now = new Date();
    const claim = {
      agentId: AGENT,
      branch: BRANCH,
    ...
    const item = queue.items.find((candidate) => candidate.id === process.env.FLIXO_WORK_ITEM_ID);
    if (!item) fail('SWARM_EVENT_STATE_CONFLICT', 'unknown work item');
    if (item.status !== 'available') fail('SWARM_EVENT_STATE_CONFLICT', `work item ${item.id} is not available`);
