...
export function simulateRepair({targetDir=process.cwd(),plan=null,maxChangedFiles=8,maxChangedLines=300}={}){
  ...
  let added=false;
  try{
    git(targetDir,['worktree','add','--detach','--quiet',dir,'HEAD'],{stdio:'pipe'});
    added=true;
    const nm=path.join(targetDir,'node_modules');
    const simNm=path.join(dir,'node_modules');
    if(fs.existsSync(nm)&&!fs.existsSync(simNm))fs.symlinkSync(nm,simNm,'dir');
    ...
    const scopeOk=summary.files.length>0&&summary.files.length<=maxChangedFiles&&summary.lines<=maxChangedLines&&summary.files.includes(plan.file);
    return Object.freeze({ok:Boolean(repair?.applied)&&scopeOk,stage:'simulation',beforeSha,repair,diff:summary,scopeOk,target:plan.file,isolated:true,reason:repair?.applied?(scopeOk?'SIMULATION_PASS':'SIMULATION_SCOPE_MISMATCH'):'SIMULATION_REPAIR_NOT_APPLIED'});
  }catch(error){
