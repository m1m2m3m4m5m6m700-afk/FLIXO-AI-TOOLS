...
const STOP_WORDS = new Set([
  ...
  'after', 'then', 'only', 'into', 'case', 'error', 'failure', 'stage',
...
function parseTeachingLine(line, sourceLine) {
...
function hintClasses({ rootCause = null, features = [] } = {}) {
  ...
  for (const value of values) {
    for (const [candidate, names] of Object.entries(aliases)) {
      if (names.some((alias) => value === alias || value.includes(alias))) hints.add(candidate);
    }
  }
  return hints;
}
