...
export function rankRepairStrategies(strategy={},memory={}){
 ...
 return Object.freeze({schemaVersion:1,inputStrategy:selected||null,attempt,deterministicNext:next,repeatPenalty,teachingEscalation:strategy.teachingEscalation===true,repairAttemptRequired:true});
}
...
export function validateReleaseIntegrity({expectedSourceSha='',executionSha='',currentExecutionSha='',prHeadSha='',mainSha='',promotionSha='',mergeSha='',mergeState='',evidenceSha='',evidenceClass='',deploymentSha=''}={}){
 const f=[];if(!sha40(expectedSourceSha))f.push('SOURCE_SHA_MISSING');if(!sha40(executionSha))f.push('EXECUTION_SHA_MISSING');if(!sha40(currentExecutionSha))f.push('CURRENT_EXECUTION_SHA_MISSING');if(executionSha&&currentExecutionSha&&executionSha!==currentExecutionSha)f.push('EXECUTION_HEAD_MISMATCH');if(prHeadSha&&executionSha&&prHeadSha!==executionSha)f.push('PR_HEAD_MISMATCH');if(mainSha&&promotionSha&&mainSha!==promotionSha)f.push('PROMOTION_MAIN_MISMATCH');if(executionSha&&promotionSha&&promotionSha!==executionSha)f.push('PROMOTION_EXECUTION_MISMATCH');if(promotionSha&&mergeSha&&promotionSha!==mergeSha)f.push('MERGE_SHA_MISMATCH');if(mergeState&&mergeState!=='success')f.push('MERGE_NOT_SUCCESS');if(evidenceSha&&executionSha&&evidenceSha!==executionSha)f.push('EVIDENCE_SHA_MISMATCH');if(evidenceClass&&evidenceClass!=='PRIMARY_EXECUTION')f.push('EVIDENCE_CLASS_INVALID');if(promotionSha&&!mainSha)f.push('PROMOTION_MAIN_MISSING');if(promotionSha&&!mergeSha)f.push('MERGE_SHA_MISSING');if(promotionSha&&!deploymentSha)f.push('DEPLOYMENT_IDENTITY_MISSING');const parent=parentOf(executionSha);if(expectedSourceSha&&parent&&parent!==expectedSourceSha)f.push('SOURCE_NOT_EXECUTION_PARENT');
...
function guard(){
 const strategy=readJson(STRATEGY,{});const memory=readJson(process.env.FLIXO_REPAIR_MEMORY??'diagnostics/auto-repair/memory.json',{cases:[]});const map=readJson(MAP,{});
 const strategyDecision=rankRepairStrategies(strategy,memory);const cover=minimizeTestSet(changedFiles(),map);const security=securityGuardian({changedFiles:changedFiles(),branch:git(['branch','--show-current'])});
