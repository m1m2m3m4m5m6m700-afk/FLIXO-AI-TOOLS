...
export function validateReleaseIntegrity({expectedSourceSha='',executionSha='',currentExecutionSha='',prHeadSha='',mainSha='',promotionSha='',mergeSha='',mergeState='',evidenceSha='',evidenceClass='',deploymentSha='',canonicalSha='',requireSourceParent=true}={}){
 const f=[];if(!sha40(expectedSourceSha))f.push('SOURCE_SHA_MISSING');if(!sha40(executionSha))f.push('EXECUTION_SHA_MISSING');if(!sha40(currentExecutionSha))f.push('CURRENT_EXECUTION_SHA_MISSING');if(executionSha&&currentExecutionSha&&executionSha!==currentExecutionSha)f.push('EXECUTION_HEAD_MISMATCH');if(prHeadSha&&executionSha&&prHeadSha!==executionSha)f.push('PR_HEAD_MISMATCH');if(mainSha&&promotionSha&&mainSha!==promotionSha)f.push('PROMOTION_MAIN_MISMATCH');if(executionSha&&promotionSha&&promotionSha!==executionSha)f.push('PROMOTION_EXECUTION_MISMATCH');if(promotionSha&&mergeSha&&promotionSha!==mergeSha)f.push('MERGE_SHA_MISMATCH');if(mergeState&&mergeState!=='success')f.push('MERGE_NOT_SUCCESS');if(evidenceSha&&executionSha&&evidenceSha!==executionSha)f.push('EVIDENCE_SHA_MISMATCH');if(evidenceClass&&evidenceClass!=='PRIMARY_EXECUTION')f.push('EVIDENCE_CLASS_INVALID');if(promotionSha&&!mainSha)f.push('PROMOTION_MAIN_MISSING');if(promotionSha&&!mergeSha)f.push('MERGE_SHA_MISSING');if(promotionSha && deploymentSha && deploymentSha!==promotionSha)f.push('DEPLOYMENT_IDENTITY_MISMATCH');
 if(promotionSha && !deploymentSha && canonicalSha!==promotionSha)f.push('DEPLOYMENT_IDENTITY_MISSING');const parent=parentOf(executionSha);if(requireSourceParent&&expectedSourceSha&&parent&&parent!==expectedSourceSha)f.push('SOURCE_NOT_EXECUTION_PARENT');
