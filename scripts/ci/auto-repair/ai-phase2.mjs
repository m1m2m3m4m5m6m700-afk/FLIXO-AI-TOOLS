...
const mode=process.argv.includes('--guard')?'guard':process.argv.includes('--release-handoff')?'release-handoff':process.argv.includes('--release')?'release':' ';if(mode==='guard')guard();else if(mode==='release-handoff')releaseHandoff();else if(mode==='release')runRelease();else{console.error('Usage: --guard|--release-handoff|--release');process.exit(2);}).test(file);
}
