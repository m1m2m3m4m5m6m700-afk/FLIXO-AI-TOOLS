...
const CANONICAL_UNIFIED_PROMPT_ID = 'RPR-UNIFIED-EXECUTION-001';
...
const buildAgentPrompt = (fingerprint, members, index) => {
  ...
  const promptHandoff = registryPromptId
    ...
    : null;
  const prompt = bindCanonicalPrompt([
    `Priority: ${index + 1}`,
    ...
    `Registry Prompt ID: ${registryPromptId ?? CANONICAL_UNIFIED_PROMPT_ID}`,
    `Canonical Prompt Path: ${CANONICAL_UNIFIED_PROMPT_PATH}`,
    '',
    external
      ? 'External-blocker context: prove provider/model/quota/network/deployment causality on this exact SHA. Do not mutate source to mask an external blocker.'
      : 'Repair context: repair the demonstrated root cause on execution only; do not weaken tests, suppress failures, blind-retry, or create a parallel control path.',
    '',
    'FAILURE EVIDENCE',
    readText(primary.log) || 'No log text captured; use referenced run/check evidence and fail closed if evidence remains insufficient.',
    '',
    'KNOWN MEMORY / REUSE',
    JSON.stringify({
...
const blockerPrompts = externalBlockers.map((blocker, index) => ({
  ...
  blocker,
  registryPromptId: CANONICAL_UNIFIED_PROMPT_ID,
  prompt: bindCanonicalPrompt([
    `Prompt kind: BLOCKED_EXTERNAL`,
    `Exact execution SHA: ${executionSha}`,
...
const taskFallback = (() => {
  ...
  for (const line of lines) {
    ...
    return {
      ...
      registryPromptId: CANONICAL_UNIFIED_PROMPT_ID,
      prompt: bindCanonicalPrompt([
        `Prompt kind: TASK_EXECUTION`,
        `Exact execution SHA: ${executionSha}`,
        `Task section: ${section}`,
        `Task: ${item[1].trim()}`,
        '',
        'Task context: execute only within declared scope, consume current evidence, implement the smallest complete change, verify the affected graph, and do not close without canonical GREEN evidence.',
      ].join('\\n')),
    };
  }
  return null;
...
const automaticVisits = ACTION_BOTS.flatMap((botId) =>
  VISIT_MODES.map((mode, visitIndex) => ({
    ...
    registryPromptId: CANONICAL_UNIFIED_PROMPT_ID,
    prompt: bindCanonicalPrompt([
      `Prompt kind: ACTION_VAULT_VISIT`,
      `Bot: ${botId}`,
      ...
      'Use shared intelligence, historical action evidence, teaching corpus, lessons and anti-lessons as advisory context only.',
      mode === 'DISCOVER'
        ? 'Discover current evidence, historical analogies, causal candidates and missing proof.'
        : mode === 'CHALLENGE'
          ? 'Challenge the current RCA and repair hypothesis; identify contradictions, stale evidence, anti-lessons or safer minimal scope.'
          : 'Record the result, lesson and anti-lesson and keep the task open until canonical GREEN.',
    ].join('\\n')),
  })),
);
...
const masterPrompt = bindCanonicalPrompt([
  ...
  `Unique incident contexts: ${prompts.length}`,
  ...
  '',
  ...allPrompts.map((item) => `### CONTEXT ${item.promptId}\\n${item.prompt}`),
].join('\\n'));
...
const bundle = {
  ...
  masterPrompt,
  canonicalPromptId: CANONICAL_UNIFIED_PROMPT_ID,
  canonicalPromptPath: CANONICAL_UNIFIED_PROMPT_PATH,
  promptSourceCount: 1,
  ...
  canonicalPromptDigest: sha256(canonicalExecutionPrompt),
};
