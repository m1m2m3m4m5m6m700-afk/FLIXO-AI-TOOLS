...
const buildAgentPrompt = (fingerprint, members, index) => {
  ...
  const prompt = [
    ...
    '5. Search Error Memory before choosing a repair hypothesis; treat memory as advisory, never as proof.',
    '6. Reproduce the failure on the exact SHA where feasible. State UNKNOWN_RCA rather than guessing.',
    '7. Apply the smallest complete root-cause repair in the declared scope on execution only.',
    ...
    '12. Write the lesson/anti-lesson so the next visit can reuse or reject this strategy.',
    '',
    'STOP CONDITIONS',
