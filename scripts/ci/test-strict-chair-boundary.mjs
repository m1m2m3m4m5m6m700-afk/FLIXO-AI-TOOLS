...
assert.ok(sync.includes('name: FLIXO Execution Canonical Sync — Chair Proposal Only') && sync.includes('CHAIR_HANDOFF_REQUIRED=true'), 'Execution Sync must remain proposal-only and require Chair handoff');
assert.match(history,/historical-action-error-index/u);
assert.match(history,/test-historical-action-error-index\.mjs/u);
assert.match(history,/contents:\s*read/u);
