...
const REQUIRED_FIELDS = ['id', 'version', 'gate', 'name', 'dependencies', 'inputs', 'outputs', 'evaluator', 'scope', 'severity', 'execution', 'reusable', 'retry', 'freshness', 'escalation'];

function fail(message) { throw new Error(`CI contract registry INVALID: ${message}`); }
...
for (const contract of CI_CONTRACTS) {
  for (const field of REQUIRED_FIELDS) if (!(field in contract)) fail(`${contract.id}: missing ${field}`);
  ...
  if (typeof contract.escalation?.deep !== 'boolean' || typeof contract.escalation?.full !== 'boolean') fail(`${contract.id}: invalid escalation flags`);
}
