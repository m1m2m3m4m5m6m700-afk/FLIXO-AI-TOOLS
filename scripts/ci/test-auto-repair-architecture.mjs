...
const taskLedger = fs.readFileSync('scripts/ci/auto-repair-task-ledger.mjs', 'utf8');
const heartbeatSource = fs.readFileSync('.github/workflows/agent-repair-heartbeat.yml', 'utf8');
const masterRepair = fs.readFileSync('scripts/ci/master-repair-orchestrator.mjs', 'utf8');
const masterRepairTest = fs.readFileSync('scripts/ci/test-master-repair-orchestrator.mjs', 'utf8');
const learningRuntime = fs.readFileSync('scripts/ci/auto-repair-learning.mjs', 'utf8');
...
assert.match(dispatcher, /group: flixo-continuous-error-watch-\$\{\{ github\.ref_name \}\}/);
assert.match(dispatcher, /cancel-in-progress: true/);
assert.match(dispatcher, /gh workflow run auto-repair\.yml/);
...
assert.doesNotMatch(heartbeat, /agent-repair-supervisor\.yml/);
assert.doesNotMatch(heartbeat, /actions\/workflows\/daily-flixo-green-gate\.yml\/dispatches/);
assert.doesNotMatch(heartbeat, /actions\/workflows\/auto-repair\.yml\/dispatches/);
assert.doesNotMatch(heartbeat, /actions:\s*write/);
assert.match(heartbeat, /actions:\s*read/);
assert.match(heartbeat, /WAKE_ALL_AGENTS|ONE_PULSE_WAKE_SCOPE=ALL_AGENTS/);
assert.match(heartbeat, /RESIDENT_SLEEP=false/);
assert.match(heartbeat, /RESIDENT_IDLE=false/);
assert.match(heartbeat, /cron: '\*\/5 \* \* \* \*'/);
assert.match(heartbeat, /workflow_dispatch:/);
assert.match(heartbeat, /cancel-in-progress: false/);
assert.match(heartbeatSource, /agent-liveness-protocol\.mjs validate/);
assert.match(masterRepair, /FLIXO-MASTER-REPAIR-ORCHESTRATOR-v1/);
assert.match(masterRepair, /READ_ONLY_MASTER_REPAIR_GATE/);
assert.match(masterRepair, /MASTER_REQUIRES_FIVE_PASSED_FALSIFICATION_CHECKS/);
assert.match(masterRepairTest, /MASTER_REPAIR_ORCHESTRATOR_TEST=PASS/);
assert.match(workflow, /name: Master Repair Gate — synthesize evidence, falsification and knowledge before mutation/);
assert.match(workflow, /FLIXO-MASTER-REPAIR-ORCHESTRATOR-v1/);
assert.match(workflow, /MASTER_REPAIR_GATE=PASS/);
assert.match(workflow, /name: Escalate blocked Master Repair dossier to Master consult/);
assert.match(workflow, /cell-master-consult\.mjs/);
assert.match(workflow, /name: Publish Master Repair dossier/);
...
assert.match(taskLedger, /FLIXO-AUTO-REPAIR-TASK-LEDGER-v1/);
assert.match(taskLedger, /integritySha256/);
assert.match(taskLedger, /SESSION_STARTED/);
assert.match(taskLedger, /SESSION_FINALIZED/);
assert.match(learningRuntime, /repairTasks/);
assert.match(learningRuntime, /maxRepairTasks: 2000/);
assert.doesNotMatch(workflow, /action-file-selection-intelligence\.mjs \\\s+node scripts\/ci\/action-file-selection-intelligence\.mjs/);
assert.doesNotMatch(workflow, /test -s \/tmp\/flixo-repair-training\.json\s+test -s \/tmp\/flixo-repair-training\.json/);
assert.doesNotMatch(workflow, /TARGET_SHA=''\s+REVERTED_COMMIT=''\s+fi\s+fi/);
assert.doesNotMatch(workflow, /AUTO_REPAIR_PUBLICATION=BLOCKED_BY_CHAIR1'\s+exit 1\s+exit 1/);
