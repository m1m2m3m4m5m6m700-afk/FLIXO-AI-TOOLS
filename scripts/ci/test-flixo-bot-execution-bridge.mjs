...
assert.match(integrator, /const publishRuntimeState = \(\): void =>/u);
assert.match(integrator, /publishRuntimeState\(\);/u);
assert.match(integrator, /finalizeFlixoBotGatewayRuntime\(runtime, 'plan'/u);
...
assert.match(ui, /prepareExecution\(deterministic\.executionPlan\);/u);
assert.doesNotMatch(ui, /prepareExecution\(deterministic\.executionPlan, \{ runtimeRequest: contextualCommand \}\)/u);
