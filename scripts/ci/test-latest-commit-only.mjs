...
for (const file of requiredWorkflows) {
  ...
  if (workflowRunConsumer) {
    ...
    assert.match(source, /(?:github\.event\.pull_request\.head\.sha \|\| github\.sha|EXPECTED_SHA|LIVE_SHA|OBSERVED_SHA|LIVE_OBSERVED_SHA|HEARTBEAT_EXACT_SHA=)/u, file + ': exact SHA binding missing');
