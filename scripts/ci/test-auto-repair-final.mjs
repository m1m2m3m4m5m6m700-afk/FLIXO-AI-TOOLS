...
assert.match(dailyGateWorkflow, /group:\s*flixo-continuous-error-watch-\$\{\{\s*github\.ref_name\s*\}\}/);
...
assert.match(handoffGateWorkflow, /branches: \[main\]/);
assert.match(handoffGateWorkflow, /contents:\s*write/);
assert.match(handoffGateWorkflow, /git\/refs\/heads\/execution/);
assert.match(handoffGateWorkflow, /--method PATCH/);
assert.match(handoffGateWorkflow, /-F force=false/);
assert.match(handoffGateWorkflow, /execution-head-authority\.mjs verify/);
assert.match(handoffGateWorkflow, /FLIXO-CHAIR-PUBLICATION-HANDOFF-v1/);
assert.match(handoffGateWorkflow, /flixo-auto-repair-publication-\$\{\{ env\.REPAIR_RUN_ID \}\}/);
assert.doesNotMatch(handoffGateWorkflow, /git\/refs\/heads\/main/);
