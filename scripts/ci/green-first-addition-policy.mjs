...
const latestWorkflow = (name) => {
  ...
  return selectLatestCanonicalRun(workflowRuns.filter(r => r?.name === name && r?.head_sha === parentSha));
};
const checkRuns = runGhJson('repos/' + repository + '/commits/' + parentSha + '/check-runs?per_page=100').check_runs ?? [];
const latestCheck = (name) => selectLatestCanonicalRun(checkRuns.filter(r => r?.name === name && r?.head_sha === parentSha));
