...
if (!sourceSha || !browser || !Number.isInteger(shard) || shard < 1) {
  throw new Error('Matrix evidence identity is incomplete.');
}
...
if (!plannedSuites.length || !plannedTestIds.length || !Number.isInteger(plannedTestCount) || plannedTestCount < 1) {
  throw new Error(`Matrix plan unit is incomplete: ${browser}:shard-${shard}`);
}
if (plannedTestIds.length !== plannedTestCount || new Set(plannedTestIds).size !== plannedTestIds.length) {
  throw new Error(`Matrix plan unit test IDs/count are inconsistent: ${browser}:shard-${shard}`);
}
...
if (JSON.stringify(plannedTestIds) !== JSON.stringify(observedTestIds)) throw new Error(`Native test inventory mismatch for ${browser}:shard-${shard}`);
if (tests.length !== plannedTestCount) throw new Error(`planned_test_count=${plannedTestCount} != observed_test_count=${tests.length}`);
if (skipped !== 0 || unexpected !== 0 || failed !== 0 || flaky !== 0) {
  throw new Error(`Native Playwright result is not clean: failed=${failed}, skipped=${skipped}, unexpected=${unexpected}, flaky=${flaky}`);
}

let playwrightVersion = null;
try {
  playwrightVersion = JSON.parse(readFileSync('node_modules/playwright/package.json', 'utf8')).version;
} catch {
