...
const base = env('FLIXO_WP_BASE_SHA', process.argv[2] ?? '');
const head = env('FLIXO_WP_HEAD_SHA', process.argv[3] ?? 'HEAD');
...
const rows = run(['log', '--format=%H%x09%aI%x09%s', '--reverse', `${base}..${head}`])
  .split('\n')
  .filter(Boolean)
  .map((x) => {
    const [sha, date, subject] = x.split('\t');
    return { sha, time: Date.parse(date), subject };
  });
...
const out = {
  schemaVersion: 1,
  policy: 'ONE_WORK_PACKAGE_PER_REPAIR_BURST',
  baseSha: base,
  headSha: head,
  ...
  bursts: bursts.map((x) => ({
    ...
    firstSha: x[0].sha,
    lastSha: x.at(-1).sha,
    workPackageIds: burstWorkPackages(x),
    allTagged: x.every((c) => markerSingle.test(c.subject)),
  })),
