...
assert.equal(teamPulse.mode,'FULL_REPOSITORY_READ_ONLY_SCAN');
...
assert.match(heartbeatWorkflow,/workflow_run:/u);
assert.match(heartbeatWorkflow,/FLIXO Agent Repair Heartbeat/u);
assert.match(heartbeatWorkflow,/cancel-in-progress:\s*false/u);
assert.match(heartbeatWorkflow,/CONTINUOUS_RESIDENCY_REARM/u);
assert.match(sessionSource,/SESSION_SHA_CHANGED/u);\nassert.match(heartbeatWorkflow,/cron:\s*'\\*\\/5 \\* \\* \\* \*'/u);
assert.match(heartbeatWorkflow,/github\.event_name.*schedule[\\s\\S]*SOURCE_BRANCH=.*execution/u);
assert.match(heartbeatWorkflow,/workflow run execution-bot-watchdog\.yml.*--ref main/u);
const watchdogWorkflow=fs.readFileSync(path.resolve(process.cwd(),'.github/workflows/execution-bot-watchdog.yml'),'utf8');
assert.match(watchdogWorkflow,/Ensure resident heartbeat is awake/u);
assert.match(watchdogWorkflow,/workflow run agent-repair-heartbeat\.yml.*--ref execution/u);
assert.match(watchdogWorkflow,/github\.event_name == 'schedule'/u);
