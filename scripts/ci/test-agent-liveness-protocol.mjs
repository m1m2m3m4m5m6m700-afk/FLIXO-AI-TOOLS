...
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotCount, 120);
...
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds.length, 120);
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds[0], 'CELL-001');
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds[119], 'CELL-120');
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentDomains.length, 10);
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentProfiles.length, 120);
assert.equal(new Set(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentProfiles.map(x=>x.botId)).size, 120);
...
assert.equal(teamPulse.logicalBotCount,120);
assert.equal(teamPulse.residentRuntimeCount,10);
assert.equal(teamPulse.logicalBotIds.length,120);
assert.equal(teamPulse.logicalBotIds[0],'CELL-001');
assert.equal(teamPulse.logicalBotIds[119],'CELL-120');
assert.equal(teamPulse.developmentProfiles.length,120);
...
assert.equal(green.fiveBotResidency.journeyLogicalBotCount,120);
assert.equal(green.fiveBotResidency.journeyCohortCount,24);
...
assert.equal(rotationPlan.logicalBotCount,120);
...
assert.equal(rotationPlan.fiveBotResidency.journeyLogicalBotCount,120);
assert.equal(rotationPlan.fiveBotResidency.journeyCohortCount,24);
...
assert.equal(pulseReport.logicalBotCount,120);
...
assert.equal(pulseReport.fiveBotResidency.journeyLogicalBotCount,120);
assert.equal(pulseReport.fiveBotResidency.journeyCohortCount,24);
...
assert.equal(pulseReport.logicalBotIds.length,120);
assert.equal(pulseReport.pulses[0].logicalBotIds.length,120);
...
assert.match(heartbeatWorkflow,/ACTIVE_BOT_COHORT_COUNT=24/u);
assert.match(heartbeatWorkflow,/LOGICAL_BOT_CYCLE=120_TO_1/u);
