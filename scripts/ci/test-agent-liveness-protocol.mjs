...
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotCount, 100);
...
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds.length, 100);
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds[0], 'CELL-001');
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotIds[99], 'CELL-100');
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentDomains.length, 10);
assert.equal(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentProfiles.length, 100);
assert.equal(new Set(AGENT_LIVENESS_PROTOCOL.logicalBotDevelopmentProfiles.map(x=>x.botId)).size, 100);
...
assert.equal(AGENT_LIVENESS_PROTOCOL.fiveBotResidencyCommitment.journeyLogicalBotCount, 100);
assert.equal(AGENT_LIVENESS_PROTOCOL.fiveBotResidencyCommitment.journeyCohortCount, 20);
...
assert.equal(teamPulse.logicalBotIds.length,100);
assert.equal(teamPulse.logicalBotIds[0],'CELL-001');
assert.equal(teamPulse.logicalBotIds[99],'CELL-100');
...
assert.equal(green.fiveBotResidency.journeyLogicalBotCount,100);
assert.equal(green.fiveBotResidency.journeyCohortCount,20);
...
assert.equal(rotationPlan.logicalBotCount,100);
...
assert.equal(rotationPlan.fiveBotResidency.journeyLogicalBotCount,100);
assert.equal(rotationPlan.fiveBotResidency.journeyCohortCount,20);
...
assert.equal(evaluateHandoff({fromCohortIndex:0,toCohortIndex:1,readyBotIds:[],elapsedMs:15*60*1000,targetSha:sha}).status,'WAIT_NEXT_COHORT_READY');
assert.equal(evaluateHandoff({fromCohortIndex:0,toCohortIndex:1,readyBotIds:cohortMembers(1),elapsedMs:15*60*1000,targetSha:sha}).status,'HANDOFF_COMMITTED');
...
assert.equal(pulseReport.logicalBotCount,100);
...
assert.equal(pulseReport.fiveBotResidency.journeyLogicalBotCount,100);
assert.equal(pulseReport.fiveBotResidency.journeyCohortCount,20);
...
assert.equal(pulseReport.logicalBotIds.length,100);
assert.equal(pulseReport.pulses[0].logicalBotIds.length,100);
...
assert.match(heartbeatWorkflow,/ACTIVE_BOT_COMMITMENT_MINUTES=15/u);
assert.match(heartbeatWorkflow,/ACTIVE_BOT_COHORT_COUNT=20/u);
assert.match(heartbeatWorkflow,/LOGICAL_BOT_CYCLE=100_TO_1/u);
...
assert.match(heartbeatWorkflow,/ACTIVE_COHORT_COMMITMENT_SECONDS=900/u);
