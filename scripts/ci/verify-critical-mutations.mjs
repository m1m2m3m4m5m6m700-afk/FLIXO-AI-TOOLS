...
if (!Array.isArray(registry.mutations) || registry.mutations.length === 0) throw new Error('critical mutation registry is malformed or empty');
...
for (const mutation of mutations) {
  if (!mutation.target_file || !mutation.project || !mutation.grep || !Array.isArray(mutation.replacement) || mutation.replacement.length !== 2) throw new Error(`${mutation.id}: executable mutation definition is incomplete`);
  if (!/^src\//.test(mutation.target_file)) throw new Error(`${mutation.id}: mutation target must be inside src/`);
}
const run = (command, args, env = {}) => spawnSync(command, args, { cwd: root, env: { ...process.env, ...env }, encoding: 'utf8', stdio: 'inherit' });
const results = [];
for (const mutation of mutations) {
  ...
  try {
    ...
    if (fs.readFileSync(target, 'utf8') === original) throw new Error(`${mutation.id}: mutation produced no source change`);
    ...
    results.push({ id: mutation.id, owner: mutation.owner, target: mutation.target, project: mutation.project, status: killed ? 'KILLED' : 'SURVIVED' });
    if (!killed) throw new Error(`${mutation.id}: critical mutation SURVIVED`);
  } finally {
    fs.writeFileSync(target, original);
  }
  if (fs.readFileSync(target, 'utf8') !== original) throw new Error(`${mutation.id}: mutation harness failed to restore ${mutation.target_file}`);
}
...
const evidence = { schema_version: 2, authority: 'CRITICAL_MUTATION_EFFECTIVENESS', status: survived === 0 && killed === mutations.length ? 'PASS' : 'FAIL', total: results.length, killed, survived, results };
