...
if (exists('docs/ASSISTANT-AGENT-COOPERATION-CONTRACT.json')) {
  try {
    ...
    const rules = ['command','truth','evidence','delegation','checkpoint','challenge','independent_review','feedback','handoff','stop','verification','learning','decision_trace','fresh_state','parallelism','arbitration','architecture','quality','efficiency','recovery','security','release','no_implicit_authority','cell_lab'];
    ...
    for (const role of ['assistantController','codeScout','errorAgent','executionAgent','reviewAgent','testAgent','securityAgent','performanceAgent','certificationAuthority','actionRepairBot','actionRepairVerifier','actionHistorian']) if (typeof contract?.roles?.[role] !== 'string') failures.push(`COOPERATION_ROLE_MISSING=${role}`);
    const roster = contract?.operationalOwnership;
    if (roster?.schemaVersion !== 1 || roster?.status !== 'ACTIVE' || roster?.policy !== 'ONE_CANONICAL_AGENT_OWNER_PER_OPERATION') failures.push('OPERATIONAL_OWNERSHIP_CONTRACT_INVALID');
    const residentIds = Array.isArray(roster?.residentAgents) ? roster.residentAgents.map((agent) => agent?.agentId) : [];
    const requiredResidents = ['assistantController','actionRepairBot','testAgent'];
    for (const id of requiredResidents) if (!residentIds.includes(id)) failures.push(`OPERATIONAL_RESIDENT_MISSING=${id}`);
    const owners = roster?.operationOwners && typeof roster.operationOwners === 'object' ? roster.operationOwners : {};
    const requiredOperations = ['build','targetedVerification','browserVerification','RCA','repositoryScouting','repair','security','certification','releasePreparation','finalIntegrationAndPublicationDecision','livenessAndRecovery'];
    for (const operation of requiredOperations) if (!String(owners[operation] ?? '').trim()) failures.push(`OPERATION_OWNER_MISSING=${operation}`);
    if (owners.build !== 'testAgent' || owners.targetedVerification !== 'testAgent' || owners.browserVerification !== 'testAgent') failures.push('TEST_AGENT_BUILD_VERIFY_OWNERSHIP_INVALID');
    if (owners.RCA !== 'errorAgent' || owners.repositoryScouting !== 'codeScout') failures.push('RCA_SCOUT_OWNERSHIP_INVALID');
    if (owners.repair !== 'actionRepairBot') failures.push('REPAIR_OWNERSHIP_INVALID');
    if (owners.security !== 'securityAgent' || owners.certification !== 'certificationAuthority') failures.push('SECURITY_CERTIFICATION_OWNERSHIP_INVALID');
    if (owners.finalIntegrationAndPublicationDecision !== 'assistantController') failures.push('FINAL_INTEGRATION_OWNERSHIP_INVALID');
    if (roster?.continuity?.heartbeatSeconds !== 60 || roster?.continuity?.noSleepWhileOpenWork !== true || roster?.continuity?.noIdleWhileOpenWork !== true) failures.push('OPERATIONAL_CONTINUITY_INVALID');
    if (!Array.isArray(roster?.conditionalAgents) || !roster.conditionalAgents.some((agent) => agent?.agentId === 'assistantRepairAgent')) failures.push('FALLBACK_AGENT_NOT_CONDITIONAL');
    if (!Array.isArray(roster?.retiredPools) || !roster.retiredPools.includes('CELL-001..CELL-200')) failures.push('RETIRED_CELL_POOL_NOT_REGISTERED');
