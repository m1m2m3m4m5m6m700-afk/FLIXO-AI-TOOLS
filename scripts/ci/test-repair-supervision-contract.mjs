...
assert.match(watchdog, /push:\s*\n\s*branches: \[execution\]/);
assert.match(watchdog, /workflow_run:\s*[\s\S]*types: \[completed\]/);
assert.match(watchdog, /github\.event_name == 'schedule'/);
assert.match(watchdog, /github\.event_name == 'push' && github\.ref == 'refs\/heads\/execution'/);
assert.match(watchdog, /github\.event_name == 'workflow_run'/);
...
assert.match(watchdog, /name: Dispatch canonical Green Gate on the first RED/);
...
assert.match(watchdog, /actions\/workflows\/daily-flixo-green-gate\.yml\/dispatches/);
assert.match(watchdog, /-f ref=main/);
...
assert.match(heartbeatWorkflow, /RESIDENT_MODE=RESIDENT_READ_ONLY_SCAN/);
assert.match(heartbeatWorkflow, /RESIDENT_STATE=READY_RESIDENT/);
