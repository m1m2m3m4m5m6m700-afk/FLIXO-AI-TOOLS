...
assert.match(heartbeatWorkflow, /logicalBotCount:100/);
assert.match(heartbeatWorkflow, /residentRuntimeCount:10/);
assert.match(heartbeatWorkflow, /action-repair-five-workers\.mjs --role=wake/);
assert.match(heartbeatWorkflow, /action=="WAKE_ALL_ACTION_REPAIR_TEAM"/);
assert.doesNotMatch(heartbeatWorkflow, /action=="WAKE_ACTION_REPAIR_SQUAD"/);
...
assert.match(heartbeatWorkflow, /Single guardian lane: trusted Watchdog owns privileged renewal/);
assert.match(heartbeatWorkflow, /WORKTREE_PATH="\/tmp\/flixo-execution-source"/);
assert.match(heartbeatWorkflow, /git worktree add --detach/);
assert.match(heartbeatWorkflow, /five-bot-handoff-gate\.mjs/);
assert.match(heartbeatWorkflow, /ACTIVE_COHORT_HANDOFF_GATE=FIVE_READY_EXACT_SHA/);
assert.doesNotMatch(heartbeatWorkflow, /actions\/workflows\/execution-bot-watchdog\.yml\/dispatches/);
