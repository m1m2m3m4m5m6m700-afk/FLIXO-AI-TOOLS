...
for (const marker of ['canonical-verify:', 'fast-contract:', 'build:', 'evidence-ledger:', 's4-runtime-e2e:']) if (!ci.includes(marker)) failures.push(`ci.yml missing canonical owner: ${marker}`);
...
if (legacyFullMatrix && /(^|\n)\s*(pull_request|push):/.test(legacyFullMatrix)) failures.push('Legacy Matrix Promotion workflow must not define its own PR/push surface.');
if (!fullMatrix) {
