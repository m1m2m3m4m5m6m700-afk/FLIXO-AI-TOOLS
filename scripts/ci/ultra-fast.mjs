...
function runCheck(check) {
  return new Promise((resolve) => {
    ...
    const child = spawn(check.command, check.args, { cwd: process.cwd(), env: process.env, stdio: ['ignore', 'pipe', 'pipe'] });
    let stdout = '';
    let stderr = '';
    let timedOut = false;
    let settled = false;
    const finish = (code, signal, error = null) => {
      ...
      const output = `${stderr}\n${stdout}`.trim();
      resolve({ id: check.id, contract: check.contract, status: code === 0 && !timedOut ? 'PASS' : 'FAIL', exitCode: code, signal, timedOut, durationMs: Date.now() - started, startedAt, finishedAt: new Date().toISOString(), command: [check.command, ...check.args].join(' '), expected: { exitCode: 0, maxDurationMs: timeoutMs }, actual: { exitCode: code, signal, timedOut }, assertion: check.assertion, source: `scripts/ci/ultra-fast.mjs:${check.id}`, output: { stdout: stdout.slice(-TAIL), stderr: stderr.slice(-TAIL), signature: output.replace(/https?:\/\/[^\s]+/giu, '<URL>').replace(/[0-9a-f]{7,64}/giu, '<SHA>').replace(/\b\d+(?:\.\d+)?\b/gu, '<N>').replace(/\s+/gu, ' ').trim().slice(-800) } });
    };
    ...
    const timer = setTimeout(() => { timedOut = true; stderr += `\nUltra Fast timeout after ${timeoutMs}ms`; killChild(child); }, timeoutMs);
    timer.unref();
  });
}

// Ultra owns detection/classification only. Canonical validators are selected later from the ownership graph.
const CHECKS = [
  { id: 'diff-check', contract: 'CI-ULTRA-FAST-001', command: 'git', args: BASE_SHA ? ['diff', '--check', `${BASE_SHA}...HEAD`] : ['diff', '--check'], timeoutMs: 30_000, assertion: 'Git diff is syntactically clean; no canonical test is rerun here.' },
];
...
async function main() {
  ...
  const environment = { node: process.version, platform: process.platform, arch: process.arch, runnerOs: process.env.RUNNER_OS ?? null, workflow: process.env.GITHUB_WORKFLOW ?? null, runId: process.env.GITHUB_RUN_ID ?? null, runAttempt: process.env.GITHUB_RUN_ATTEMPT ?? null };
  const startedAt = new Date().toISOString();
  const results = await Promise.all(CHECKS.map(runCheck));
  const events = results.filter((result) => result.status === 'FAIL').map((result) => ({ id: result.id, contract: result.contract, status: 'FAIL', message: result.output.signature || `${result.command} failed`, source: result.source, durationMs: result.durationMs }));
  const failureIntelligence = events.length ? aggregateFailures(events) : { schemaVersion: 1, rootCauses: [], failures: [], unknownCount: 0, reportHash: hash('empty') };
