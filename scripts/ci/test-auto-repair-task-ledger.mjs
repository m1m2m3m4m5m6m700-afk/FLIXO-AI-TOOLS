...
appendTaskEvent(loaded,{type:'VERIFICATION_CAPTURED',detail:{exactSha:true}});
writeTaskLedger(file,loaded);
const reloaded=loadTaskLedger(file);
assert.equal(reloaded.events.at(-1).type,'VERIFICATION_CAPTURED');
assert.match(reloaded.integritySha256,/^[a-f0-9]{64}$/u);
