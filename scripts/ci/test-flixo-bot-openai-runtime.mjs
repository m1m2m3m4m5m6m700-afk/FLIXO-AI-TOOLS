...
const called = recordToolCall(running, SHA_A, request, { mutationAuthority: false, certificationAuthority: false });
assert.equal(called.decision.allowed, true);
assert.equal(called.state.events.at(-1)?.type, 'TOOL_CALL');

assert.throws(
  () => recordToolResult(called.state, SHA_A, running.currentOwner, nested.toolId, true, { finding: 'unverified source' }),
...
assert.throws(
  () => approveRun(approved, SHA_A, approval.pendingApproval.approvalId),
  /FLIXO_BOT_APPROVAL_INVALID_STATE/,
);

const evidenced = recordToolResult(
  approved, SHA_A, approved.currentOwner, nested.toolId, true, { byteLength: 10, mimeType: 'image/png' }, {
    inputSha256: '1'.repeat(64), outputSha256: '2'.repeat(64), verified: true,
    receiptChainSha256: '3'.repeat(64), executorId: nested.toolId, executionMode: 'LOCAL', attempt: 0,
  },
);
...
const limited = startRun(createFlixoBotRunState({
  taskId: 'TURN-LIMIT', agentId: 'ACTION-REPAIR', exactSha: SHA_A, request: 'bounded turns', maxTurns: 1,
}), SHA_A);
assert.equal(applyNextStep(limited, SHA_A, { type: 'RUN_AGAIN', reason: 'budget test' }).status, 'FAILED');
...
assert.equal(cancelRun(running, SHA_A, 'operator stop').status, 'CANCELLED');
