...
assert.match(contract, /preparation specialist/i);
assert.match(contract, /Preparation Contract/i);
...
assert.match(agent, /NO_DIRECT_MUTATION/);
assert.match(agent, /PREPARATION_ONLY/);
assert.match(agent, /TASK_PREPARATION_ONLY/);
...
assert.match(agent, /PREPARATION_HANDOFF_PENDING_EXECUTION/);
...
assert.match(agent, /CONTRACT_VERSION = 'TASK-AGENT-PREPARATION-v3'/);
...
assert.match(execution, /status: 'PREPARATION_HANDOFF_PENDING'/);
assert.doesNotMatch(agent, /openNewCycleForEveryRedCheck/);
...
assert.match(agent, /applyAuthority: 'EXECUTION_AGENT_OR_REPAIR_AGENT'/);
assert.doesNotMatch(agent, /TASK_AGENT_DIRECT_EXECUTION|TASK_AGENT_ON_EXECUTION_BRANCH_ONLY/);

console.log(JSON.stringify({ status: 'PASS', authority: 'TASK_AGENT_CONTRACT_TEST', checks: 31 }, null, 2));
