...
export function buildKnowledgeRecord(input={}){
 ...
 const personalMemory=botId?learnIntoBotMemory({botId,taskId,taskShortName,taskName:input.taskName??null,knowledge:{id:'CK-'+sha256([taskId,fingerprint,runId].join('|')).slice(0,24),claim,solution,taskIdentity:{version:Math.max(1,Number(input.taskVersion)||1)}},verification:input.verification,evidenceRef:input.evidenceRef??null,outcome,rootCause,rule,solution,weakness,upgrade:botUpgrade}):null;
