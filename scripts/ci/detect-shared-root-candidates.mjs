...
for (let i = 0; i < roots.length; i += 1) {
  for (let j = i + 1; j < roots.length; j += 1) {
    ...
    candidates.push({ id: `SRC-${[a.id, b.id].sort().join('-')}`, roots: [a.id, b.id].sort(), confidence: Number(Math.min(0.95, evidence).toFixed(2)), basis: { sharedFiles: files.slice(0, 30), sharedChecks: checks.slice(0, 30), sharedSymptoms: symptoms.slice(0, 30), sharedCategories: categories }, classification: evidence >= 0.75 ? 'HIGH' : 'CANDIDATE', action: evidence >= 0.75 ? 'INVESTIGATE_SHARED_ROOT_BEFORE_SYMPTOM_PATCHING' : 'COLLECT_MORE_EVIDENCE' });
  }
}
const output = { schema: 'flixo-shared-root-candidates/v1', generatedAt: new Date().toISOString(), candidateCount: candidates.length, highConfidenceCount: candidates.filter((item) => item.classification === 'HIGH').length, candidates, operatingRule: 'NO SYMPTOM PATCHING WHEN SHARED ROOT IS DETECTED' };
