...
export function buildRootCauseProof({targetSha=null,failureFingerprint=null,failedRunId=null,diagnosis=null,strategy=null,fileSelection=null,failureLog=''}={}){
 ...
 const evidenceRecords=[
  {id:'EV-RCA-FAILURE',source:'failureLog',detail:String(failureLog).slice(0,1000)},
  {id:'EV-RCA-ROOT',source:'selectedHypothesis',detail:mechanism.evidenceLines},
  {id:'EV-RCA-LOCATION',source:'diagnosis.location.file',detail:location},
  {id:'EV-RCA-INVARIANT',source:'causalGraph.violatedInvariant',detail:graph.violatedInvariant},
 ].map(item=>({...item,targetSha,failureFingerprint,runId:failedRunId,timestamp:new Date().toISOString()}));
