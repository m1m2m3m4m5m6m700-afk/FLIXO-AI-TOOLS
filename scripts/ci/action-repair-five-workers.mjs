...
if(role==='wake'){
 ...
 if(!['PUSH_READY','RED_INTERNAL','BLOCKED_EXTERNAL','FAIL_CLOSED','RESIDENT_READY','ACTIVE_OPERATION','ACTIONABLE_RED_REENTRY'].includes(status))throw new Error('ACTION_WAKE_STATUS_INVALID');
 const teamIds=FLIXO10_IDS;
 const rotation=buildFiveBotRotation({targetSha,now:new Date().toISOString(),cohortIndexOverride:arg('cohort-index')||null,cycleNumberOverride:arg('cycle-number')||null});
 const result={schemaVersion:3,botId:'FLIXO_HEARTBEAT_CONTROLLER',role:'CANONICAL_WAKE_CONTROLLER',action:'WAKE_ALL_ACTION_REPAIR_TEAM',wakeScope:'ALL_ACTION_REPAIR_TEAM',wakePolicy:'ANY_ACTIVE_ACTION_REPAIR_BOT_WAKES_ALL',dispatcher:'FLIXO_AGENT_REPAIR_HEARTBEAT',targetRunId:runId,targetSha,failureFingerprint:wakeFingerprint,status,mutationAuthority:false,directDispatch:false,dispatchRequired:false,dispatchWorkflow:null,actionRepairSquadReady:true,residentState:status==='RESIDENT_READY'?'READY_RESIDENT':'ACTIVE_OPERATION',sleep:false,idle:false,withdrawal:false,seatContinuity:{minimumConnectedSeats:5,heartbeatAckRequired:true,generatedWakeIsNotAttendanceProof:true,lazyBotAction:'IMMEDIATE_REPLACE_FROM_STAGED_OR_VERIFIED_PROVISIONED_RUNTIME',attendanceCertification:'PENDING_LIVE_HEARTBEAT_ACK'},recipientCount:teamIds.length,recipients:teamIds,activeCohortCount:rotation.cohortCount,activeCohortSize:rotation.activeBotCount,activeBotIds:rotation.activeBotIds,stagedBotIds:rotation.stagedBotIds,nextBotIds:rotation.nextBotIds,activeRuntimeCount:rotation.activeRuntimeCount,activeRuntimeIds:rotation.activeRuntimeIds,stagedRuntimeCount:rotation.stagedRuntimeCount,stagedRuntimeIds:rotation.stagedRuntimeIds,nextCohortReady:true,nextCohortReadyCount:rotation.handoff.requiredReadyCount,handoffPolicy:rotation.handoff.policy,fiveBotResidency:rotation.fiveBotResidency,seatContinuity:{minimumConnectedSeats:5,heartbeatAckRequired:true,generatedWakeIsNotAttendanceProof:true,lazyBotAction:'IMMEDIATE_REPLACE_FROM_STAGED_OR_VERIFIED_PROVISIONED_RUNTIME',attendanceCertification:'PENDING_LIVE_HEARTBEAT_ACK'},sourceMutationOwner:'ACTION-REPAIR',pushAuthority:'CHAIR_1_ONLY',sharedReferences:SHARED_REFS,canonicalNextStep:status==='PUSH_READY' || status==='RED_INTERNAL' || status==='ACTIONABLE_RED_REENTRY'?'DAILY_FLIXO_GREEN_GATE':status==='RESIDENT_READY'?'RESIDENT_READY_RECONCILIATION':'CONTINUE_ACTIVE_OPERATION'};
 appendActionCenterEvent({type:'WAKE',taskId:'ACTION-WAKE:'+runId+':'+wakeFingerprint,fingerprint:wakeFingerprint,runId,targetSha,actor:'ACTION-WAKE',payload:{status,dispatcher:result.dispatcher,wakeScope:result.wakeScope,recipientCount:result.recipientCount,recipients:result.recipients,dispatchRequired:result.dispatchRequired,dispatchWorkflow:result.dispatchWorkflow,canonicalNextStep:result.canonicalNextStep}});
 fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));process.exit(0);
}
...
const rotation=buildFiveBotRotation({targetSha,now:new Date().toISOString(),cohortIndexOverride:arg('cohort-index')||null,cycleNumberOverride:arg('cycle-number')||null});
const packets=FLIXO10_IDS.map((id,i)=>{const opponent=nextFlixo(id);const active=i<rotation.activeBotCount;const logicalBotId=active?rotation.activeBotIds[i]:rotation.nextBotIds[i-rotation.activeBotCount];return {schemaVersion:3,workerId:id,workerIndex:i+1,logicalBotId,active,cohortRole:active?'ACTIVE_COHORT':'NEXT_COHORT_READY',readySignal:!active,readyExactSha:!active?targetSha:null,readyAt:!active?new Date().toISOString():null,role:'FLIXO_TEAM',lifecycle:active?'AWAKE_RESIDENT_PENDING_HEARTBEAT_ACK':'STAGED_READY',wakeSource:'FLIXO_AGENT_REPAIR_HEARTBEAT',resident:true,attendanceStatus:active?'AWAITING_LIVE_HEARTBEAT_ACK':'STAGED_READY',heartbeatAck:false,heartbeatAt:null,ringPosition:i+1,taskReceiptRole:'PROPOSER_AND_RING_MEMBER',adversaryBot:opponent,adversaryRole:'DYNAMIC_ON_TASK_RECEIPT',legacyRole:Object.entries(ROLE_MAP).find(([,value])=>value===['ACTION_REPAIR_TWIN_A','ACTION_REPAIR_TWIN_B','ACTION_SOLUTION_INDEXER_SUPPORT','ACTION_BEST_OPTION_SELECTOR','ACTION_RCA_EVIDENCE_REVIEW','ACTION_BLAST_RADIUS_REVIEW','ACTION_SECURITY_BOUNDARY_REVIEW','ACTION_REGRESSION_PLANNER','ACTION_CERTIFIER_ADVERSARY','ACTION_FINAL_CERTIFIER'][i])?.[0]??null,systemScope:'FULL_REPOSITORY_AND_AUTOMATION_SYSTEM',workflow:'READ→DIAGNOSE→WRITE_PROPOSAL→RING_CHALLENGE→HANDOFF',cloneSource:'FLIXO-BOT-SYSTEM-WIDE-INTELLIGENCE',intelligenceVersion:'FLIXO-BOT-BRAIN-v1',cognitiveParity:'EXACT_SHARED_BY_REFERENCE',taskId:'ACTION-REPAIR:'+runId+':'+fingerprint,target:{runId,failedSha:targetSha,failureFingerprint:fingerprint,logDigest},sharedReferences:SHARED_REFS,sameIncidentContext:true,mutationAuthority:false,canonicalMutationOwner:'DYNAMIC_FLIXO10_PUSH_SEAT',pushSeatEligible:true,pushAuthority:'FIRST_CONNECTED_FLIXO10_ONLY',proofRule:'CURRENT_EXACT_SHA_CI_ONLY'};});
const report={schemaVersion:3,authority:'ACTION_REPAIR_SQUAD_FANOUT',lifecycle:'AWAKE_RESIDENT',wakeSource:'FLIXO_AGENT_REPAIR_HEARTBEAT',targetSha,runId,failureFingerprint:fingerprint,residentWorkerCount:10,workerCount:10,activeWorkerCount:rotation.activeBotCount,stagedWorkerCount:5,activeBotIds:rotation.activeBotIds,nextBotIds:rotation.nextBotIds,activeRuntimeCount:rotation.activeRuntimeCount,activeRuntimeIds:rotation.activeRuntimeIds,stagedRuntimeCount:rotation.stagedRuntimeCount,stagedRuntimeIds:rotation.stagedRuntimeIds,cohortCount:rotation.cohortCount,cohortIndex:rotation.cohortIndex,cycleNumber:rotation.cycleNumber,commitmentWindowMs:rotation.commitmentWindowMs,handoffPolicy:rotation.handoff.policy,fiveBotResidency:rotation.fiveBotResidency,nextCohortReady:packets.filter(worker=>worker.active===false&&worker.readySignal===true&&worker.readyExactSha===targetSha&&worker.heartbeatAck===true&&worker.attendanceStatus==='READY').length===5,workers:packets,sharedReferences:SHARED_REFS,sameReferencesForAll:true,anyActionFailureAdmitted:true,roleOrder:FLIXO10_IDS,mutationModel:'TEN_CLONED_FLIXO_BOT_ROLES_ONE_CANONICAL_MUTATION_LANE',wakeModel:'EXISTING_CANONICAL_DISPATCHER_ONLY',selectionModel:'TEN_WORKER_RING_EVIDENCE_TO_ACTION-REPAIR',ringModel:'NEXT_BOT_IS_DYNAMIC_ADVERSARY_ON_TASK_RECEIPT',cloneModel:'ONE_SHARED_COGNITIVE_KERNEL_WITH_ROLE_OVERLAYS',pushAuthority:'CHAIR_1_ONLY',sourceMutationOwner:'ACTION-REPAIR',logEvidencePresent:logExists,generatedAt:new Date().toISOString()};
