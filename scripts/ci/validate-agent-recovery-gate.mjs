...
for (const marker of ['DIRECT_EXECUTION','DIRECT_ON_ISOLATED_REPAIR_BRANCH','DIRECT_SOURCE_MUTATION_COMMIT_PUSH_ON_REPAIR_BRANCH','مهام.md']) if (!read('scripts/ci/task-agent.mjs').includes(marker)) throw new Error(`task-agent:${marker}`);
for (const marker of ['singleOrchestrator: true','specializedRolesAreStages: true','failClosed: true','mainBranchMutation: false']) if (!read('scripts/ci/agent-execution-control.mjs').includes(marker)) throw new Error(`execution:${marker}`);
const sha = execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
console.log(JSON.stringify({status:'PASS',schemaVersion:2,baselineSha:sha,runtimeModel:'TASK_AGENT_DIRECT_EXECUTION_ON_ISOLATED_REPAIR_BRANCH_PLUS_SINGLE_ORCHESTRATOR',roles:'STAGES_WITH_DIRECT_REPAIR_EXECUTION',historicalRecovery:'CLASSIFIED_NOT_COPIED_WHOLESALE',failClosed:true,mainBranchMutation:false},null,2));
