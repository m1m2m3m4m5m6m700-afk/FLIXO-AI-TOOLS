...
async function main(){
  ...
  const env={...process.env,FLIXO_TARGET_DIR:root,FLIXO_EXPECTED_TARGET_SHA:expectedSha,FLIXO_PATCH_BASE_SHA:parentSha};
  ...
  await new Promise(resolve=>{
    ...
    const settle=(name,value)=>{
      outcomes[name]=value;
      if(value.code!==0&&firstFailure===null){firstFailure=name;if(name==='targetedRegression')killGroup(assessor.child);else killGroup(regression.child);}
  ...
  const bothPass=outcomes.targetedRegression?.code===0&&outcomes.adversarial?.code===0&&adversarial?.status==='NO_COUNTEREXAMPLE';
  const aggregate={
    ...
    gate:{targetedRegression:outcomes.targetedRegression?.code===0,adversarialNoCounterexample:outcomes.adversarial?.code===0&&adversarial?.status==='NO_COUNTEREXAMPLE',result:bothPass?'PASS':'FAIL'},
    targetedRegression:{code:outcomes.targetedRegression?.code??1,signal:outcomes.targetedRegression?.signal??null},
    adversarial:{code:outcomes.adversarial?.code??1,signal:outcomes.adversarial?.signal??null,status:adversarial?.status??(outcomes.adversarial?.code===0?'MISSING_EVIDENCE':'ABORTED'),targetSha:adversarial?.targetSha??null},
