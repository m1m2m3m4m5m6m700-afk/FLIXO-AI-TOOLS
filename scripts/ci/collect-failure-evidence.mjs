...
const expectedSha=(process.env.EXPECTED_SHA??context?.runtime?.expectedSha??context?.identity?.expectedSha??'').trim();
const currentSha=git(['rev-parse','HEAD']);
const exactSha=Boolean(
  /^[0-9a-f]{40}$/iu.test(expectedSha) &&
  expectedSha===context?.execution?.sha &&
...
const bundle={schema:'flixo-failure-evidence/v4',generatedAt:new Date().toISOString(),sha:currentSha,expectedSha,executionIdentityHash:context?.identityHash??null,reports:reports.map(({name,report})=>({gate:name,present:Boolean(report),status:report?.status??'MISSING',sha:report?.sha??null,checksExpected:report?.checksExpected??null,checksExecuted:report?.checksExecuted??null,failures:report?.failures??null,rootCauses:report?.rootCauses??[],artifactSha256:hash(`${name.toLowerCase()}.json`)})),failures,skipped,completeness:{requiredGates,missingReports:missing,incompleteReports:incomplete.map(r=>r.name),allRequiredReportsPresent:missing.length===0,allExpectedChecksExecuted:incomplete.length===0,overallCoherent:coherent,authoritative},invariants:{exactShaMatch:exactSha,expectedShaPresent:/^[0-9a-f]{40}$/iu.test(expectedSha),headSha:currentSha,cleanCheckout:context?.execution?.dirty===false,requiredTestsSkipped:skipped.length>0,maskedFailures:masked,staleEvidence:reports.some(({report})=>report?.sha&&report.sha!==context?.execution?.sha)}};
