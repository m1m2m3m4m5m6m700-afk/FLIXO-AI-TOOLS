...
const browserIndependent = Array.isArray(testPlan?.gates?.browser?.projects) && testPlan.gates.browser.projects.length === testPlan.gates.browser.expected && testPlan.gates.browser.projects.every((project) => runner.includes(`--project=${project}`));
findings.push(finding('RC-BROWSER-INTELLIGENCE-001', 'TEST_INTELLIGENCE', browserIndependent ? 'CLEAR' : 'HIGH', browserIndependent ? 'RESOLVED' : 'LATENT_CI_DEBT', browserIndependent ? 'Browser engines are independently defined by the shared test plan and exposed as individual checks.' : 'Browser engines are not independently represented by the shared test plan and runner.', browserIndependent ? 'test-plan.json defines Chromium, Firefox and WebKit; test.mjs executes each project independently.' : 'The browser project list and runner execution graph are inconsistent.', 'Keep engine-level checks independent and source their count from the shared test plan.'));
...
findings.push(finding('RC-CI-COVERAGE-SOT-001', 'TEST_COVERAGE', coverageSot ? 'CLEAR' : 'HIGH', coverageSot ? 'RESOLVED' : 'LATENT_CI_DEBT', coverageSot ? 'Gate/check counts are defined once in the shared test plan and consumed by runner and evidence collection.' : 'Expected gate/check counts remain duplicated or lack a shared test-plan contract.', coverageSot ? 'scripts/ci/test-plan.json is consumed by scripts/test.mjs and collect-failure-evidence.mjs.' : 'Shared test plan is incomplete or not consumed by all required components.', 'Keep test-plan.json as the only source of expected gate/check counts.'));
