...
const tasks = parseTasks(source); const requested = arg('task-id'); const allReady = process.argv.includes('--all-ready');
const selected = requested ? tasks.filter((task) => task.taskId === requested || task.title.includes(requested)) : allReady ? tasks.filter((task) => !task.completed) : tasks.filter((task) => !task.completed).slice(0, 1);
if (!selected.length) throw new Error(requested ? `TASK_NOT_FOUND=${requested}` : 'NO_READY_TASKS');
fs.mkdirSync(OUTPUT_DIR, { recursive: true });
const generatedAt = new Date().toISOString(); const outputs = [];
for (const task of selected) {
  const fingerprint = hash(`${task.taskId}|${task.title}|${task.section}`).slice(0, 16);
  const packet = {
    schemaVersion: 5, authority: 'FLIXO_TASK_AGENT', role: 'TASK_OWNER_AND_REPAIR_EXECUTOR', mode: 'REPAIR_BRANCH_EXECUTION', preparedOnly: false,
    mutationPolicy: 'REPAIR_BRANCH_ONLY_NO_DIRECT_MAIN_MUTATION', actionOwnership: { actions: true, sourceFiles: true, workflows: true, commit: true, push: true, directMainPush: false, autoMerge: 'CANONICAL_GREEN_ONLY' },
    taskFile: 'مهام.md', task, baselineSha: sha, repairBranch: branch, generatedAt, errorFingerprint: fingerprint,
    repairSummary: { status: 'ACTIVE', taskId: task.taskId, fingerprint, error: 'UNOBSERVED', rootCause: 'UNOBSERVED', repair: 'EXECUTE_ON_REPAIR_BRANCH', verification: 'PENDING' },
    instructions: { objective: 'Understand, implement, verify, commit and push the task on the repair branch; never mutate main directly.', verificationRequired: true, unresolvedWorkMustBeReported: true },
    completionPolicy: { stateAfterRepair: 'REPAIR_PENDING_VERIFICATION', stateAfterAnyRedCheck: 'REPAIR_PENDING', stateAfterGreenCheck: 'REVERIFY_ALL', terminalState: 'CLOSED_VERIFIED_ONLY_AFTER_CANONICAL_GREEN', everyRepairOpensAnotherVerificationCycle: true, everyRedCheckMustBecomeARepairTarget: true, newlyIntroducedFailuresMustOpenNewCycles: true, taskCannotBeClosedFromTargetedRegressionAlone: true },
    repairLoop: { mode: 'RED_TO_GREEN', maxCycles: 12, rescanAfterEveryRepair: true, rescanScope: 'ALL_REQUIRED_CHECKS', circuitBreaker: { enabled: true, maxStalledCycles: 3, definition: 'SAME_FAILURE_FINGERPRINT_WITHOUT_VERIFIABLE_PROGRESS', action: 'REQUIRES_REVIEW', failClosed: true }, closureGate: ['canonical-ci-green', 'zero-red-checks', 'fresh-exact-sha-evidence', 'required-regression-proof'] },
