...
for (const item of queue.items) {
  ...
  for (const dep of item.dependsOn) if (dep === item.id || !byId.has(dep)) fail(`unknown/self dependency ${dep} on ${item.id}`);
}
...
for (const item of queue.items) {
  for (const other of queue.items) if (item.id !== other.id) {
    ...
    if (sharedContract || sharedRootCause) {
      if (item.ownerAgentId !== other.ownerAgentId) fail(`logical ownership collision: ${item.id} <-> ${other.id}`);
    }
  }
}
