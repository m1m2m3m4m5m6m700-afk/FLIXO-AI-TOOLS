...
function verifyReference(rawValue: string, file: string): void {
  ...
  if (!fs.existsSync(target)) {
    fail(`${path.relative(ROOT, file)}: unresolved local asset reference: ${rawValue}`);
  }
}
...
function scanHtml(file: string, html: string): void {
  ...
  for (const match of html.matchAll(/(?:href|xlink:href)\s*=\s*["']#([^"']+)["']/giu)) {
    ...
    if (!new RegExp(`\\bid=["']${escapedId}["']`, 'u').test(html)) {
      fail(`${path.relative(ROOT, file)}: SVG fragment #${id} has no matching id definition in the built document`);
    }
  }
}
