...
const TEST_PLAN = JSON.parse(readFileSync(resolve(ROOT, 'scripts/ci/test-plan.json'), 'utf8'));
const EXPECTED = Object.fromEntries(GATES.map((name) => [name, TEST_PLAN.gates[name].expected]));
const BROWSER_PROJECTS = TEST_PLAN.gates.browser.projects;
...
const CHECKS = {
  ...
  build: [['build','npm',['run','build'],'RC-BUILD-001'],['dist','node',['-e',"const fs=require('node:fs'); for(const p of ['dist','dist/index.html']) if(!fs.existsSync(p)) throw new Error('Missing build output: '+p);"],'RC-BUILD-001'],['performance-budget','npm',['run','validate:performance-budget'],'RC-BUILD-001']],
};
for (const [name, checks] of Object.entries(CHECKS)) if (checks.length !== EXPECTED[name]) { console.error(`TEST PLAN DRIFT: ${name} expected ${EXPECTED[name]}, runner defines ${checks.length}`); process.exit(1); }
function execute(label, command, commandArgs, env = {}) { const startedAt = now(); const result = spawnSync(command, commandArgs, { cwd: ROOT, env: { ...process.env, ...env }, encoding: 'utf8' }); const output = `${result.stdout ?? ''}\n${result.stderr ?? ''}`.slice(-16000); process.stdout.write(result.stdout ?? ''); process.stderr.write(result.stderr ?? ''); return { label, command: [command, ...commandArgs].join(' '), status: result.status === 0 ? 'PASS' : 'FAIL', exitCode: result.status ?? 1, startedAt, completedAt: now(), output }; }
...
function enrich(gateName, check, declared) { const id = classify(gateName, check, declared); if (check.status === 'PASS') return { ...check, rootCauseId: null, fingerprint: null, repro: null, error: null }; const fingerprint = `FPR-${createHash('sha256').update([id,gateName,check.label,signature(check)].join('\n')).digest('hex').slice(0,12).toUpperCase()}`; return { ...check, rootCauseId: id, fingerprint, repro: gateName === 'browser' ? `npx playwright test --project=${check.label}` : check.command, error: { category: rootCauses[id]?.category ?? 'UNKNOWN', normalized: normalize(check.output).slice(-4000) } }; }
function report(gateName, results, status) { const checks = results.map((item) => enrich(gateName, item.result, item.rootCause)); const failures = checks.filter((check) => check.status === 'FAIL'); const report = { version: 5, schema: 'flixo-gate-report/v5', sha: sha(), gate: gateName.toUpperCase(), mode, status, failures: failures.length, checksExpected: EXPECTED[gateName], checksExecuted: checks.length, rootCauses: [...new Set(failures.map((check) => check.rootCauseId).filter(Boolean))], completedAt: now(), checks }; writeFileSync(resolve(DIR, `${gateName}.json`), `${JSON.stringify(report, null, 2)}\n`); return report; }
function runChecks(gateName) { const results = []; for (const [label, command, commandArgs, rootCause] of CHECKS[gateName]) { const result = execute(label, command, commandArgs); results.push({ result, rootCause }); if (mode === 'certification' && result.status === 'FAIL') break; } return report(gateName, results, results.length === EXPECTED[gateName] && results.every((item) => item.result.status === 'PASS') ? 'PASS' : 'FAIL'); }
async function waitForServer(url) { for (let attempt = 0; attempt < 40; attempt += 1) { try { const response = await fetch(url); if (response.ok) return; } catch { /* preview server not ready yet */ } await new Promise((resolveSleep) => setTimeout(resolveSleep, 500)); } throw new Error(`Preview server did not become ready: ${url}`); }
async function browserGate() { const server = spawn('npm',['run','preview','--','--host','127.0.0.1','--port','3000'],{cwd:ROOT,env:{...process.env,CI:'true'},stdio:'ignore'}); try { await waitForServer('http://127.0.0.1:3000'); const results = []; for (const project of BROWSER_PROJECTS) { const result = execute(project,'npx',['playwright','test',`--project=${project}`],{CI:'true',S4_EXTERNAL_SERVER:'true',PLAYWRIGHT_REUSE_SERVER:'false'}); results.push({ result, rootCause: 'RC-BROWSER-001' }); if (mode === 'certification' && result.status === 'FAIL') break; } return report('browser', results, results.length === EXPECTED.browser && results.every((item) => item.result.status === 'PASS') ? 'PASS' : 'FAIL'); } finally { server.kill('SIGTERM'); } }
function blocked() { return report('browser', [], 'BLOCKED'); }
function overall(reports, target) { const failures = reports.flatMap((report) => (report.checks ?? []).filter((check) => check.status === 'FAIL')); const roots = [...new Set(failures.map((check) => check.rootCauseId).filter(Boolean))]; const pass = reports.length === target.length && reports.every((report) => report.status === 'PASS' && report.checksExpected === report.checksExecuted) && failures.length === 0; const output = { version: 5, schema: 'flixo-ci-report/v5', mode, sha: sha(), status: pass ? 'PASS' : 'FAIL', gatesExpected: target.length, gatesExecuted: reports.length, rootCauses: roots, firstFailure: failures[0] ? { rootCauseId: failures[0].rootCauseId, fingerprint: failures[0].fingerprint, repro: failures[0].repro } : null, completedAt: now() }; writeFileSync(resolve(DIR, 'report.json'), `${JSON.stringify(output, null, 2)}\n`); writeFileSync(resolve(DIR, 'failures.json'), `${JSON.stringify(failures, null, 2)}\n`); return output; }
...
const target = gate ? [gate] : GATES;
const reports = [];
for (const name of target) {
  const report = name === 'static' ? runChecks('static') : name === 'build' ? runChecks('build') : target.length === 1 ? await browserGate() : reports.find((item) => item.gate === 'BUILD')?.status === 'PASS' ? await browserGate() : blocked();
  reports.push(report);
