...
if (!['certification', 'diagnose'].includes(mode) || (requestedGate && !GATES.includes(requestedGate))) process.exit(2);
if (!Number.isInteger(MAX_CONCURRENCY) || MAX_CONCURRENCY < 1 || MAX_CONCURRENCY > 32) process.exit(2);
...
function enrich(gateName, check) {
  ...
  const id = classify(gateName, check, check.rootCause);
  const fingerprint = `FPR-${createHash('sha256').update([id, gateName, check.label, signature(check)].join('\n')).digest('hex').slice(0, 12).toUpperCase()}`;
  return { ...check, rootCauseId: id, fingerprint, repro: gateName === 'browser' ? `npx playwright test --project=${check.label}` : [check.command, ...(check.args ?? [])].join(' '), error: { category: rootCauses[id]?.category ?? 'UNKNOWN', normalized: normalize(check.output).slice(-4000) } };
}

function execute(check, env = {}) {
  return new Promise((resolveResult) => {
    const startedAt = now();
    const child = spawn(check.command, check.args ?? [], { cwd: ROOT, env: { ...process.env, ...env }, stdio: ['ignore', 'pipe', 'pipe'] });
    let stdout = '';
    let stderr = '';
    ...
    child.on('error', (error) => resolveResult({ id: check.id, label: check.label, command: [check.command, ...(check.args ?? [])].join(' '), status: 'FAIL', exitCode: 1, startedAt, completedAt: now(), output: `${stdout}\n${stderr}\n${error.message}`.slice(-16000) }));
    child.on('close', (code, signal) => resolveResult({ id: check.id, label: check.label, command: [check.command, ...(check.args ?? [])].join(' '), status: code === 0 ? 'PASS' : 'FAIL', exitCode: code ?? 1, signal: signal ?? null, startedAt, completedAt: now(), output: `${stdout}\n${stderr}`.slice(-16000) }));
  });
}

async function executeMatrix(gateName, checks, env = {}) {
...
async function waitForServer(url) {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    try {
      ...
      if (attempt === 39) console.error(`Preview probe failed: ${error.message}`);
    }
...
async function browserGate() {
  const server = spawn('npm', ['run', 'preview', '--', '--host', '127.0.0.1', '--port', '3000'], { cwd: ROOT, env: { ...process.env, CI: 'true' }, stdio: 'ignore' });
  try {
    await waitForServer('http://127.0.0.1:3000');
    const browserChecks = TEST_PLAN.gates.browser.checks.map((check) => ({ ...check, dependencies: [] }));
    const results = await executeMatrix('browser', browserChecks, { CI: 'true', S4_EXTERNAL_SERVER: 'true', PLAYWRIGHT_REUSE_SERVER: 'false' });
    return buildReport('browser', results);
  } finally {
    server.kill('SIGTERM');
  }
}

function overall(reports, target) {
...
const context = runNode('scripts/ci/capture-execution-context.mjs');
...
for (const script of ['scripts/ci/normalize-reproduction.mjs', 'scripts/ci/collect-failure-evidence.mjs', 'scripts/ci/record-repair-cycle.mjs', 'scripts/ci/detect-shared-root-candidates.mjs']) {
  const diagnostic = runNode(script);
  process.stdout.write(diagnostic.stdout ?? '');
  process.stderr.write(diagnostic.stderr ?? '');
  if ((diagnostic.status ?? 1) !== 0) console.error(`Supporting diagnostic ${script} returned ${diagnostic.status ?? 1}.`);
}
