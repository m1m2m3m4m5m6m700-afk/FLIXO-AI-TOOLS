...
if (!Number.isInteger(MAX_CONCURRENCY) || MAX_CONCURRENCY < 1 || MAX_CONCURRENCY > 32) { console.error(`Invalid test-plan maxConcurrency: ${TEST_PLAN.execution?.maxConcurrency}`); process.exit(2); }
...
for (const gate of GATES) {
  if (CHECKS[gate].length !== EXPECTED[gate]) { console.error(`TEST PLAN DRIFT: ${gate} expected ${EXPECTED[gate]}, matrix defines ${CHECKS[gate].length}`); process.exit(1); }
  const ids = CHECKS[gate].map((check) => check.id);
  if (new Set(ids).size !== ids.length) { console.error(`TEST PLAN DRIFT: duplicate check id in ${gate}`); process.exit(1); }
}
...
for (const check of allChecks) {
  for (const dependency of check.dependencies ?? []) if (!allIds.has(dependency)) { console.error(`TEST PLAN DRIFT: ${check.id} depends on unknown check ${dependency}`); process.exit(1); }
  if (!Array.isArray(check.coverage) || check.coverage.length === 0) { console.error(`TEST PLAN DRIFT: ${check.id} has no coverage mapping`); process.exit(1); }
}
...
async function executeMatrix(gateName, checks) { const results = new Map(); const pending = new Map(checks.map((check) => [check.id, check])); const active = new Set(); while (pending.size || active.size) { const runnable = [...pending.values()].filter((check) => (check.dependencies ?? []).every((dependency) => results.has(dependency))).slice(0, Math.max(0, MAX_CONCURRENCY - active.size)); for (const check of runnable) { pending.delete(check.id); const promise = execute(check).then((result) => { results.set(check.id, enrich(gateName, { ...result, rootCause: check.rootCause, coverage: check.coverage, cost: check.cost, dependencies: check.dependencies ?? [] })); active.delete(promise); }); active.add(promise); } if (active.size === 0 && pending.size) { console.error(`TEST PLAN DEADLOCK: ${gateName} has unresolved dependency cycle or invalid ordering.`); process.exit(1); } if (active.size) await Promise.race(active); } return checks.map((check) => results.get(check.id)); }
...
const reports = [];
for (const gateName of target) { if (gateName === 'browser') { const buildReport = reports.find((report) => report.gate === 'BUILD'); if (target.length > 1 && buildReport && buildReport.status !== 'PASS') reports.push({ version: 6, schema: 'flixo-gate-report/v6', sha: sha(), gate: 'BROWSER', mode, status: 'BLOCKED', failures: 0, checksExpected: EXPECTED.browser, checksExecuted: 0, rootCauses: ['RC-BUILD-001'], coverage: [], completedAt: now(), checks: [] }); else reports.push(await browserGate()); } else reports.push(buildReport(gateName, await executeMatrix(gateName, CHECKS[gateName]))); if (mode === 'certification' && reports.at(-1).status !== 'PASS' && gateName !== 'static') break; }
