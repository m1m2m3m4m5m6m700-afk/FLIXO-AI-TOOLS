...
function compareValue(en, loc, path, context) {
  ...
  if (!normalized && !nonTranslatable.has(path.split('.').at(-1))) fail(`${context}: empty translation at ${path}`);
  if (target !== 'en' && en === loc && normalized && !allowedSame.has(normalized) && !nonTranslatable.has(path.split('.').at(-1))) {
...
// 1. Canonical runtime registry + metadata.
...
// 2. Canonical locale dictionaries: key parity, identity, non-empty values, placeholders and markup.
const localeModules = {};
for (const locale of CANONICAL_LOCALES) {
...
// 3. Automatically cover every locale-bearing data module under src.
function walk(dir) {
...
for (const absolute of candidateFiles) {
  ...
  try {
    ...
    for (const [exportName, exported] of Object.entries(module)) {
      ...
      const english = exported.en;
      const localized = exported[target];
      compareLocaleObject(english, localized, `${relativePath}:${exportName}/${target}`);
    }
  } catch (error) {
...
// 4. Tool SEO localization: every tool that exposes an SEO surface must expose all 20 locales.
...
// 5. Localization boundary policy: user-facing literals in UI source must not bypass localization.
const uiRoots = ['src/routes', 'src/components', 'src/pages', 'src/layouts', 'src/tools'];
const technicalWords = /^(?:FLIXO|QuickFlow|OCR|PDF|AI|SVG|PNG|JPG|JPEG|WEBP|MP3|MP4|Ctrl|http|https|localhost|true|false|undefined|null)$/iu;
...
for (const uiRoot of uiRoots) {
  ...
  for (const absolute of walk(dir)) {
    ...
    for (const match of source.matchAll(attrPattern)) {
      ...
      if (!/[A-Za-z]{2}/u.test(value) || technicalWords.test(value)) continue;
      if (/^(?:class|id|button|input|div|span|img|svg|path|main|nav|header|footer)$/iu.test(value)) continue;
      fail(`hard-coded UI attribute bypasses localization in ${relativePath}: ${JSON.stringify(value)}`);
    }
    for (const match of source.matchAll(jsxTextPattern)) {
      const value = normalize(match[1]);
      if (value.length < 4 || !/[A-Za-z]{2}/u.test(value) || technicalWords.test(value)) continue;
      if (/^(?:[A-Z][A-Za-z0-9_-]*|[A-Z]{2,})$/u.test(value)) continue;
      fail(`hard-coded JSX text bypasses localization in ${relativePath}: ${JSON.stringify(value)}`);
    }
  }
}

// 6. Explicit fallback markers in source: these are prohibited for non-English locales.
if (target !== 'en') {
...
console.log(`COMPLETE LOCALIZATION GATE PASSED — ${target}; canonical registry, all locale-bearing modules, tool SEO surfaces, translation contracts, fallback policy, placeholders/HTML integrity, script/direction integrity, and UI localization boundaries are clean.`);
