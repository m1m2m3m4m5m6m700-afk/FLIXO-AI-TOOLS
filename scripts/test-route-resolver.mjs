...
assert.equal(getLocalizedToolPath(tool, 'ur'), '/ur/image-compressor');
assert.equal(getLocalizedToolUrl('https://canonical.test', tool, 'ar'), 'https://canonical.test/ar/image-compressor');
assert.throws(() => getLocalizedToolPath({ path: '/image-compressor' }, 'en'), /Invalid canonical tool path/u);
