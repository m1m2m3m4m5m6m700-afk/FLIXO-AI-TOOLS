...
function countUsages(name) {
  const pattern = new RegExp(`(?:from\\s+|import\\s*\\(|require\\s*\\(|exports?\\.)['\"']${escapeRegex(name)}(?:/[^'\"']*)?['\"']|['\"']${escapeRegex(name)}['\"']`, 'g');
  const matches = [];
  for (const entry of source) {
    const count = entry.content.match(pattern)?.length ?? 0;
    if (count > 0) matches.push({ file: entry.file, count });
  }
  return matches;
}

const entries = dependencyNames.map((name) => {
  const usage = countUsages(name);
