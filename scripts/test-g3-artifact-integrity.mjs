...
const buildFixtures = async () => {
  ...
  return [
    ...
    { type: 'SVG', mime: 'image/svg+xml', extension: 'svg', content: text('<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"><rect width="1" height="1"/></svg>'), parse: 'utf8' },
...
const inspectParseability = async (entry) => {
  ...
  if (entry.parse === 'image') {
    ...
    const view = new DataView(entry.content.buffer, entry.content.byteOffset, entry.content.byteLength);
...
await runCase('malformed JSON', () => {
  ...
  const jsonEntry = matrix.find((entry) => entry.type === 'JSON');
  const result = validateOutputIntegrity(
    malformedJson.byteLength,
    'application/json',
    createSpec(jsonEntry),
    undefined,
...
await runCase('invalid UTF-8', () => {
  ...
  const textEntry = matrix.find((entry) => entry.type === 'Text');
  const result = validateOutputIntegrity(
    invalidUtf8.byteLength,
    'text/plain',
    createSpec(textEntry),
    undefined,
...
console.log('G3 image artifact integrity: PASS — complete matrix, no skips, no failures');
