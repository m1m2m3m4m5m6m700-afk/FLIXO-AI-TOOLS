...
| `RC-P00-SOURCE-INJECTION-SYNTAX-001` | STATIC_VALIDATION | P00 protocol installation inserted literal `\\n` escapes into executable JavaScript and left the task-ledger validator pinned to an obsolete heading, producing fail-closed syntax and TASK_GATE failures. | scripts/ci/agent-session.mjs, scripts/ci/validate-agent-coordination.mjs, scripts/ci/validate-agent-protocol.mjs | Exact-source syntax + P00 marker regression | Executable source uses real statement boundaries; validator targets the canonical current P00 task-ledger heading. || `RC-GREEN-GATE-PENDING-CANCELLATION-001` | CI_EVIDENCE | Daily·FLIXO Green Gate observations could be cancelled or superseded before evidence publication because concurrency grouped by branch/SHA and GitHub allows only one pending item per concurrency group. | .github/workflows/daily-flixo-green-gate.yml, scripts/ci/test-auto-repair-architecture.mjs | assert Green Gate concurrency group uses github.run_id ; assert cancel-in-progress is false |  |
