...
The complete workflow sweep found no other automatic `execution` push duplicates among the 25 workflow definitions; scheduled/manual/workflow_run surfaces remain intentionally distinct.\n\n## Bounded development while canonical GREEN is pending\n\nThe previous blanket RED lock is superseded. A RED repository may carry bounded development work when the commit is explicitly classified with `[REPAIR:<ID>] [WP:<ID>]` or `[ADD:<ID>] [WP:<ID>]`. This does not permit skipped tests, weakened security, certification manipulation, stale evidence, direct `main` mutation, third branches, or alternate mutation authorities. Canonical verification and exact-SHA evidence remain mandatory. The executable policy is `scripts/ci/green-first-addition-policy.mjs`.\n