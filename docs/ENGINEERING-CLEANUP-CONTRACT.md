...
Keep FLIXO correct, reproducible, evidenced, and release-safe while removing legacy verification and build complexity that has no independent proof boundary.
...
- `DUPLICATE`: removed only after coverage equivalence is established.
- `LEGACY-DEPENDENCY`: migrated away from consumers before deletion.
- `UNPROVEN`: never treated as release evidence until identity and scope are established.
