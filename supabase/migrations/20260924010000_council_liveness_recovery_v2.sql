...
-- P0 invariants:
--   * expired LEASED/ACKED work cannot remain open forever;
--   * recovery is bounded and emits explicit RECOVERED/BLOCKED/FAILED_TERMINAL evidence;
--   * live residency requires fresh heartbeat evidence;
--   * heartbeat carries the exact execution SHA;
--   * privileged RPCs are service_role-only.
...
declare
  v_dispatch public.flix_council_dispatches%rowtype;
begin
  if p_exact_sha !~ '^[0-9a-f]{40}$' then raise exception 'COUNCIL_EXACT_SHA_INVALID'; end if;
  ...
  values (v_dispatch.dispatch_id,p_account_id,'ACKED',p_exact_sha,jsonb_build_object('sessionId',p_session_id));
...
begin
  ...
  insert into public.flix_council_events(dispatch_id,account_id,event_type,exact_sha,payload)
  values (v_dispatch.dispatch_id,p_account_id,'HEARTBEAT',p_exact_sha,jsonb_build_object('leaseExpiresAt',v_dispatch.lease_expires_at));
...
begin
  ...
  begin
    ...
    select count(*)::integer into v_remaining_expired
      from public.flix_council_dispatches
     where status in ('LEASED','ACKED')
       and lease_expires_at is not null
       and lease_expires_at <= v_now;

    select count(*)::integer into v_stale_accounts
      from public.flix_council_accounts
     where active = true
       and coalesce(metadata->>'residencyRequired', 'false') = 'true'
       and (
         coalesce(last_heartbeat_at, last_seen_at) is null
         or coalesce(last_heartbeat_at, last_seen_at) < v_now - make_interval(secs => greatest(120, lease_seconds * 2))
       );
    ...
    update public.flixo_automation_watchdog
       set state=v_state,
           ...
           evidence=jsonb_build_object(
             ...
             'healthyRequires',jsonb_build_array('zero expired open leases','zero stale resident accounts','fresh heartbeat evidence'),
             'at',v_now,
  ...
  exception when others then
    ...
    insert into public.flixo_automation_watchdog_events(
      ...
      1,'FAILURE','DEGRADED',0,jsonb_build_object('at',v_now,'error',left(v_error,2000))
    );
