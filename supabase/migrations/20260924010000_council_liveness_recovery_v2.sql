...
begin
  if not exists (
    ...
    alter table public.flix_council_accounts
      ...
      check (current_execution_sha is null or current_execution_sha ~ '^[0-9a-f]{40}create or replace function public.council_recover_expired_dispatches(p_limit integer default 10)
...
$block$;
 then raise exception 'COUNCIL_EXACT_SHA_INVALID'; end if;
  select * into v_dispatch from public.flix_council_dispatches where dispatch_id = p_dispatch_id for update;
  ...
  update public.flix_council_dispatches
     set status = 'ACKED', acked_at = coalesce(acked_at, now()), updated_at = now()
   where dispatch_id = p_dispatch_id returning * into v_dispatch;
  update public.flix_council_accounts
     set current_session_id = p_session_id, last_seen_at = now(), last_heartbeat_at = now(),
         current_execution_sha = p_exact_sha, updated_at = now()
   where account_id = p_account_id;
...
$block$;
 then raise exception 'COUNCIL_EXACT_SHA_INVALID'; end if;
  ...
  if v_dispatch.session_id is null or v_dispatch.session_id <> p_session_id then raise exception 'COUNCIL_SESSION_MISMATCH'; end if;
  select lease_seconds into v_lease_seconds from public.flix_council_accounts where account_id = p_account_id and active = true for update;
